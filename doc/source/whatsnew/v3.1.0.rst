...
.. ipython:: python
    ...
    data = "a\n1\n1\n2"
    pd.read_csv(StringIO(data), dtype="category")["a"].dtype
...
The same inference now applies to the other readers built on the shared text
parser, including :func:`read_table`, :func:`read_fwf` and :func:`read_excel`,
...
Two further inconsistencies in the ``c`` and ``python`` engines are fixed.
...
a :class:`CategoricalDtype` that specifies ``ordered=True`` without specifying
...
.. ipython:: python
    ...
    data = "a\n2\n1"
    pd.read_csv(StringIO(data), dtype={"a": CategoricalDtype(ordered=True)})["a"].dtype
