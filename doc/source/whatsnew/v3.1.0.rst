...
- Bug in :meth:`DataFrame.isetitem` corrupting the frame's internal block structure when ``loc`` was not in increasing order, so a later :meth:`DataFrame.merge`, :meth:`DataFrame.join` or :meth:`DataFrame.describe` raised ``AssertionError`` and :meth:`DataFrame.to_pickle` wrote a file that could not be read back (:issue:`68930`)
