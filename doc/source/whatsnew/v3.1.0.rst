...
- :meth:`DataFrame.to_json` and :meth:`Series.to_json` now accept ``double_precision=None`` to write floating point values with the shortest representation that round-trips exactly (:issue:`62072`, :issue:`59313`, :issue:`23328`)
...
.. _whatsnew_310.deprecations.json_floats:

Deprecated imprecise floating point handling in JSON I/O
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

By default, :meth:`DataFrame.to_json` and :meth:`Series.to_json` round floating
point values to 10 decimal places, and :func:`read_json` parses floating point
values with a method that is not always correctly rounded. As a result, floating
point values can change when written to JSON and read back (:issue:`62464`).

In a future version, floating point values will be written with the shortest
representation that round-trips exactly, and parsed with full precision. You can
opt in to this behavior now by passing ``double_precision=None`` to ``to_json``
and ``precise_float=True`` to ``read_json``:

.. code-block:: ipython

    In [1]: df = pd.DataFrame({"a": [1 / 3]})

    In [2]: df.to_json(double_precision=None)
    Out[2]: '{"a":{"0":0.3333333333333333}}'

Passing an integer for ``double_precision`` and passing ``precise_float=False``
are deprecated. To control the number of digits written, round the data before
writing. When neither keyword is passed, a ``Pandas4Warning`` is emitted if the
data being written or read contains floating point values.
...
- Performance improvement in :func:`read_csv` with ``engine="c"`` for string columns under ``future.infer_string`` or ``dtype_backend="pyarrow"``, most of all for files with several string columns read in parallel (:issue:`68379`)
...
- Performance improvement in :func:`read_csv` with ``engine="c"`` when converting numeric columns of wide files and of parallel reads (:issue:`68380`)
...
- Performance improvement in :func:`read_csv` with ``engine="c"`` when reading large files in parallel: the number of pieces the file is split into now scales with its size rather than only the thread count, so a large file is no longer read as coarsely as a small one (:issue:`66798`)
...
- Performance improvement in :func:`read_json` with ``precise_float=True`` (:issue:`62464`)
...
- :func:`read_json` with ``engine="pyarrow"`` now raises ``ValueError`` when passed an option the engine cannot apply -- such as ``typ="series"``, ``convert_dates``, ``keep_default_dates``, ``date_unit``, ``convert_axes``, ``encoding``, ``encoding_errors``, ``storage_options``, an explicit ``compression``, or a non-``ArrowDtype`` ``dtype`` -- instead of silently ignoring it and returning an incorrect result (:issue:`66144`)
