...
- :meth:`Index.join` with ``how="left"`` or ``how="right"`` on indexes of different dtypes now keeps the dtype of the index the result is taken from, instead of casting to a common dtype such as ``object``. This carries through to :meth:`Series.align`, :meth:`DataFrame.join`, :func:`merge` and :meth:`Series.str.cat`, so e.g. ``ser.str.cat(other)`` no longer changes a ``str`` index to ``object`` (:issue:`63371`)
