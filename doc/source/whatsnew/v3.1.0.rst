...
- Bug in :meth:`MultiIndex.set_levels` and :meth:`MultiIndex.set_codes` raising a self-contradictory ``"must be list-like"`` error for a nested (list-of-lists) value with a scalar ``level``; the message now says the value must be flat, and a flat list of tuple values is no longer misread as a nested list-of-lists (:issue:`66287`)
