...
- Bug in :class:`Timestamp` constructor, :func:`to_datetime`, and :func:`read_csv` with ``parse_dates`` where a comma used as a date separator was read as a decimal point, silently giving a wrong result, e.g. ``"2013,11,04"`` parsed as ``2013-04-01`` and ``"Nov 14,2013"`` as year 1; such strings are now rejected, and :func:`read_csv` leaves the column as strings instead of parsing it. The ISO 8601 decimal comma for fractional seconds, e.g. ``"2013-11-04 01:02:03,456"``, still parses (:issue:`17265`)
