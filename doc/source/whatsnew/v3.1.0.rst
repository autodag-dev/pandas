...
- A :class:`Timestamp` pickled by pandas 1.2 or earlier that carries a ``freq`` or a ``tz`` now raises ``ValueError`` when loaded with :func:`pickle.load` directly; read such files with :func:`read_pickle`, which handles the legacy format (:issue:`31930`)
