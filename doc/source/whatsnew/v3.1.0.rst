...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem broadcasting a 1-D value down rows instead of across the selected columns on an all-``datetime64`` or all-``timedelta64`` :class:`DataFrame` (e.g. ``df.iloc[[2, 0], ::-1] = value``), writing the values to the wrong cells when the value's length matched the number of selected rows and otherwise raising ``OutOfBoundsDatetime`` instead of reporting the shape mismatch (:issue:`68521`)
