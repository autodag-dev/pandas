...
- Bug in :class:`Timestamp` constructor, :func:`to_datetime`, and :func:`read_csv` with ``parse_dates`` where strings with a ``"GMT"`` timezone name raised ``ValueError`` on systems whose local timezone is named ``GMT`` (e.g. ``Europe/London``, ``Africa/Abidjan``) instead of parsing to UTC as they do elsewhere (:issue:`58002`)
