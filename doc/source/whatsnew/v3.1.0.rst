...
- Fixed bug in :func:`read_csv` with the ``c`` engine where string values representing ``"nan"`` could raise a :class:`ValueError` during float conversion instead of being correctly parsed as missing values.
