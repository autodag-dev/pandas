...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` and ``usecols`` where the selected columns were returned in the order given in ``usecols`` instead of the file order like the other engines; the error raised for ``usecols`` not matching the columns now also matches the other engines (:issue:`67010`)
