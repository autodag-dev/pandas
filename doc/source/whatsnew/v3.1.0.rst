...
- Bug in :meth:`DataFrame.query` where an expression referencing a duplicated column label silently filtered on just one of those columns; it now raises ``ValueError`` unless the expression evaluates to a one-dimensional row mask (:issue:`65588`)
