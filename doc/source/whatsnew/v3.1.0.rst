...
- Bug in multiplying a :class:`Series` of ``timedelta64`` by an integer or float, or dividing it by a float, where an overflowing result silently wrapped or saturated instead of raising ``OutOfBoundsTimedelta``; multiplying by ``inf`` now raises ``OutOfBoundsTimedelta`` instead of returning ``NaT`` (:issue:`43178`)
