...
- ``numexpr`` 2.14.1 is no longer used to accelerate arithmetic and comparisons; some builds of that release intermittently and silently returned incorrect results, such as an all-zero column, for operations on large objects. pandas now warns at import time and falls back to NumPy, and :func:`eval`, :meth:`DataFrame.eval` and :meth:`DataFrame.query` raise ``ImportError`` when passed an explicit ``engine="numexpr"``; install ``numexpr`` 2.14.2 or newer to restore the acceleration. HDF5 ``where=`` queries are evaluated by PyTables, which imports ``numexpr`` itself, and are not covered (:issue:`65408`)
-
...
- Bug in :meth:`DataFrame.select_dtypes` where ``pd.CategoricalDtype(ordered=True)`` selected every categorical column instead of only the ordered ones, and excluding it dropped unordered categorical columns (:issue:`40234`)
...
- Bug in :meth:`DataFrame.select_dtypes` where an interval spec that gives ``closed`` but leaves the subtype open, ``pd.IntervalDtype(closed="left")``, selected every interval column instead of only the left-closed ones, excluding it dropped every interval column, and pairing it with another subtype-less spec in ``include`` and ``exclude`` raised a spurious overlap ``ValueError`` (:issue:`68491`)
- Bug in :meth:`DataFrame.select_dtypes` where an interval spec that left ``closed`` or the subtype's resolution open, such as ``"interval[int64]"``, ``"interval[datetime64]"`` or ``pd.IntervalDtype("int64")``, selected no columns; it now selects every interval column matching the components the spec does give (:issue:`40234`)
...
- Bug in arithmetic and logical operations on PyArrow-backed data raising an opaque ``pyarrow.ArrowInvalid`` or ``pyarrow.ArrowTypeError``, instead of an informative ``TypeError``, when the other operand held objects PyArrow cannot convert, such as :class:`Categorical`, :class:`Interval`, or ``DateOffset`` objects (:issue:`62682`)
- Bug in comparisons between PyArrow-backed data and objects PyArrow cannot convert raising an opaque ``pyarrow.ArrowInvalid`` or ``pyarrow.ArrowTypeError`` instead of comparing elementwise for ``==``/``!=`` and raising an informative ``TypeError`` for ordered comparisons, as the NumPy-nullable dtypes do (:issue:`62682`)
