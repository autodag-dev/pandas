...
- Bug in :meth:`DataFrame.query` and :meth:`DataFrame.eval` under ``parser="python"`` incorrectly raising ``NotImplementedError`` for ``BoolOp`` (``and``/``or``), ``In``, and ``NotIn`` AST nodes, so any expression using ``and``/``or``, membership (``in``/``not in``), a chained comparison (e.g. ``a < b < c``), or a string-column equality — including any query with a :class:`MultiIndex` — failed even though the same expression works under ``parser="pandas"``
