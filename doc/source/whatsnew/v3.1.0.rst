...
- Bug in :func:`merge_asof` raising ``MergeError`` with the message ``"tolerance must be positive"`` for a negative ``tolerance``, even though ``tolerance=0`` (exact-match-only) is accepted; the message now says ``"tolerance must be non-negative"`` (:issue:`66289`)
