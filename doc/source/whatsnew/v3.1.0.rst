...
- Bug in :func:`merge` and :meth:`DataFrame.merge` with ``how="right"`` where the shared join-key column kept the calling (left) frame's dtype instead of the right frame's, making ``a.merge(b, how="left")`` and ``b.merge(a, how="right")`` produce inconsistent dtypes for the same logical join (:issue:`56454`)
