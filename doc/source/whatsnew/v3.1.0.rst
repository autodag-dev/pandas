...
Numeric
^^^^^^^
- Bug in :meth:`DataFrame.diff` and :meth:`Series.diff` on unsigned integer dtypes (``uint8``, ``uint16``, ``uint32``, ``uint64``) silently wrapping around instead of returning a negative difference, e.g. ``uint32(0) - uint32(3)`` producing ``4294967293.0`` instead of ``-3.0``; ``uint64`` values too large to fit in ``int64`` are now diffed exactly using arbitrary-precision Python ints rather than wrapping (:issue:`4899`)
