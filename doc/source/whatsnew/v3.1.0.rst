...
- Bug in :func:`to_numeric` with ``errors="coerce"`` on :class:`ArrowDtype` input where an unparsable value was coerced by embedding a raw ``NaN`` directly in the underlying data instead of the null mask, so :meth:`Series.isna` reported it as present and :meth:`Series.ffill` left it unfilled (:issue:`67949`)
