...
- Bug in :class:`Interval`, :class:`IntervalArray`, and :class:`IntervalIndex` not accepting a ``numpy.str_`` ``closed`` (e.g. an element of a numpy string array); the array constructors took one but then raised ``TypeError`` on ``repr``, iteration, and indexing (:issue:`68078`)
