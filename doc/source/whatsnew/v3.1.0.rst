...
- Fixed bug in :func:`read_sql` with ``dtype_backend="pyarrow"`` where ``datetime.date`` and ``datetime.time`` columns were returned as ``string[pyarrow]`` instead of ``date32``/``time64``, inconsistent with ``datetime`` columns (:issue:`56551`)
