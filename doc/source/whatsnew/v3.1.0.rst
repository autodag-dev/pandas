...
- Bug in :meth:`DataFrame.loc` raising ``NotImplementedError`` when setting a single-column :class:`DataFrame` with an :class:`ExtensionArray` dtype using a boolean column indexer (e.g. ``df.loc[:, [True]]`` or ``df.loc[:, df.columns.isin(["A"])]``); a ``[False]`` mask is now correctly treated as a no-op (:issue:`66527`, :issue:`66579`)
