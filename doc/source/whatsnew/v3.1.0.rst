...
- Bug in :meth:`MultiIndex.get_locs`, :meth:`MultiIndex.get_loc_level`, :meth:`MultiIndex.slice_locs`, :meth:`DataFrame.loc` and :meth:`Series.loc` where a key with more entries than the index has levels reported a misleading lexsort-depth error or a raw ``IndexError`` instead of naming the key length (:issue:`45762`)
