...
- Fixed regression in :func:`read_csv` with ``sep=None`` where the default engine raised ``TypeError`` instead of falling back to the Python engine, while explicitly selecting the ``c`` or ``pyarrow`` engine did not raise an informative ``ValueError`` (:issue:`66639`)
