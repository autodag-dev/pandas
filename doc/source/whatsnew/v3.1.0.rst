...
- Removed ``Series._constructor_expanddim_from_mgr``, one of the hooks a :class:`Series` subclass could override to control the type returned by :meth:`Series.to_frame`. Subclasses now only need ``_constructor_expanddim``, which is passed a :class:`DataFrame`. An override left in place is no longer called (:issue:`56681`)
