...
- Bug in arithmetic, comparison, and logical operations between PyArrow-backed data and a two-dimensional operand raising an assortment of internal errors, or silently returning a wrong result; these now raise ``NotImplementedError`` (:issue:`62682`)
- Bug in comparisons between PyArrow-backed data and a differently-sized array or list raising ``pyarrow.ArrowInvalid`` instead of ``ValueError`` (:issue:`62682`)
