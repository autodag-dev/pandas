...
- Bug in :meth:`DataFrame.iloc` setitem writing only some of the targeted cells, silently ignoring a value of the wrong shape, or raising ``ValueError`` or ``IndexError``, when the column indexer was a non-increasing slice (e.g. ``::-1``) and the row indexer was a list-like such as a list, :class:`Series`, or boolean mask, on a :class:`DataFrame` whose values are referenced by another object (:issue:`65446`)
