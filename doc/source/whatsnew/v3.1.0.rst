...
- Bug in :func:`testing.assert_series_equal` and :func:`testing.assert_frame_equal` with ``check_category_order=False`` incorrectly reporting categoricals that contain missing values as different when their categories were in a different order (:issue:`62008`)
