...
- Bug in comparison operators between a :class:`SparseDtype`-backed :class:`Series` and a nullable ``boolean``, integer or floating :class:`Series` raising ``AttributeError`` for a ``boolean`` operand, and silently resolving the missing values as if they were ``NaN`` for an integer or floating one; the result now matches the non-sparse equivalent and has ``boolean`` dtype (:issue:`68579`)
