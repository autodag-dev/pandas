...
- :func:`read_json` with ``orient="table"`` now round-trips integer, float and boolean column labels and index names, such as the default integer columns of ``DataFrame([[1, 2]])``; previously the values were silently read as all-NaN or the read failed with a confusing ``IntCastingNaNError``. A label that still cannot be matched to the data it labels -- a float rounded by ``double_precision`` on the way out, or two labels sharing one key in the written data such as ``5`` and ``"5"`` or a duplicated column name -- now raises ``ValueError``; labels :meth:`DataFrame.to_json` does not write unambiguously are still not recovered: a ``None`` column label, written as the placeholder ``"values"``, reads back under that name with all-NaN values, and an index name of a different type from the column labels raises (:issue:`19129`)
...
- :meth:`DataFrame.to_json` and :meth:`Series.to_json` with ``orient="table"``, and :func:`build_table_schema`, no longer raise ``AttributeError`` for a :class:`MultiIndex` with non-string level names (:issue:`19129`)
...
- :meth:`Series.to_json` with ``orient="table"`` no longer writes a falsy :class:`Series` name such as ``0``, ``False`` or ``""`` as the placeholder ``"values"``; only an unnamed :class:`Series` gets the placeholder (:issue:`19129`)
