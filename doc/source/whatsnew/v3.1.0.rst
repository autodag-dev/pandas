...
- :meth:`DataFrame.plot` with ``kind="line"`` now draws all the columns of a wide frame as a single ``matplotlib.collections.PolyCollection`` rather than one ``Line2D`` per column. The plot itself is unchanged, but the columns are no longer individually addressable artists: they appear in ``ax.collections`` instead of ``ax.get_lines()``, ``ax.get_legend_handles_labels()`` no longer reports them (so a bare ``ax.legend()`` call after plotting finds no labelled artists), and ``ax.relim()`` cannot recompute the data limits, as matplotlib does not support collections there. Pass ``legend`` and the axis limits through :meth:`DataFrame.plot` itself, or read the drawn data off ``ax.collections``. This only applies where a collection draws the same thing as the per-column artists, so it is skipped for time-series, stacked, subplotted and secondary-y plots, for styles, markers and error bars, for property cycles carrying more than color, and for any frame long enough for matplotlib to simplify its line paths (:issue:`61532`)
...
- Deprecated ``inplace`` keyword for :meth:`DataFrame.rename`, :meth:`DataFrame.drop`, :meth:`DataFrame.dropna`, :meth:`DataFrame.drop_duplicates`, :meth:`DataFrame.sort_values`, :meth:`DataFrame.sort_index`, :meth:`DataFrame.eval`, :meth:`DataFrame.query`, :meth:`DataFrame.rename_axis`, :meth:`DataFrame.set_index`, :meth:`DataFrame.reset_index` (and the ``Series`` equivalents). This keyword will be removed in a future version (:issue:`63207`); see also `PDEP-8 <https://pandas.pydata.org/pdeps/0008-inplace-methods-in-pandas.html>`_
...
- Performance improvement in :meth:`DataFrame.plot` for line plots of wide DataFrames (:issue:`61532`)
