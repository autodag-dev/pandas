...
- Bug in :meth:`Series.to_json` and :meth:`DataFrame.to_json` raising ``ValueError`` when serializing NumPy dtypes in ``DataFrame.dtypes`` or ``DataFrame.dtypes.to_frame`` (:issue:`61170`)
