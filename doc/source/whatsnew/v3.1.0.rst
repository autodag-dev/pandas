...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` where ``usecols`` given as a list of column names returned the columns in the order they appeared in ``usecols`` instead of the file's column order, unlike the other engines (:issue:`67010`)
