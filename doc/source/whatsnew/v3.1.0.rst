...
- Bug in :meth:`DataFrame.__setitem__` silently corrupting the :class:`DataFrame`, or dropping columns of ``value``, when the key matched several columns (duplicate or :class:`MultiIndex` columns) and ``value`` was timezone-aware ``datetime64``, ``period``, or a 2D array with the wrong number of columns; it now raises ``ValueError`` (:issue:`68445`)
...
- Bug in :meth:`DataFrame.isetitem` with a sequence of positions silently corrupting the :class:`DataFrame` (which then raised ``IndexError`` on ``repr``) or dropping columns of ``value``, when ``value`` was a NumPy array without one column per position; it now raises ``ValueError`` (:issue:`68445`)
