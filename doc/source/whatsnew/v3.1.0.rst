...
timezone instead of returning a naive result; it still drops nanoseconds
...
- Bug in :class:`Timestamp` constructor where passing ``fold`` alongside date components given positionally, e.g. ``Timestamp(2020, 1, 1, tzinfo=timezone.utc, fold=0)``, raised ``ValueError``; this also made :func:`copy.replace` raise on a ``fold=1`` timestamp (:issue:`52117`)
