...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem spuriously transposing a two-dimensional value on an all-``datetime64`` or all-``timedelta64`` :class:`DataFrame` when neither the row nor the column key was a slice (e.g. ``df.iloc[[2, 0], [0, 2]] = value``); depending on the shapes this wrote the wrong cells, raised ``OutOfBoundsDatetime``, or accepted a value whose shape the equivalent NumPy-dtype frame rejects (:issue:`65446`)
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem writing the wrong cells or raising ``IndexError`` or ``ValueError`` on a :class:`DataFrame` whose values are referenced by another object, where the same assignment on an unreferenced :class:`DataFrame` succeeded (e.g. ``df.iloc[[2, 0], ::-1] = value``) (:issue:`65446`)
