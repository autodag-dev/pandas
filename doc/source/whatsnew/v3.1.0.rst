...
- Bug in :func:`qcut` not validating ``q``: ``q=0`` silently returned an all-``NaN`` result, a negative ``q`` raised ``IndexError`` or a confusing ``ValueError`` naming a number of samples, and a ``q`` that was neither an integer nor list-like (e.g. ``True``, ``2.0`` or ``None``) raised ``TypeError`` from an internal call; these now raise ``ValueError``, as :func:`cut` already did for ``bins`` (:issue:`67406`)
