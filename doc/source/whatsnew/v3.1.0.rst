...
- Deprecated setting values with :meth:`DataFrame.loc` using a tuple key that does not match any existing key of a :class:`MultiIndex` on ``axis=0`` whose number of levels equals the frame's number of dimensions, when at least one of the tuple's per-axis interpretations (row, column) would expand the frame. The tuple is currently reinterpreted as ``(row, column)`` indexers; in a future version it will always be treated as a :class:`MultiIndex` row key instead. Use ``df.loc[key, :]`` to index by row and column explicitly (:issue:`65326`)
