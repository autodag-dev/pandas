...
- Bug in :meth:`Index.difference` returning an empty result when ``other`` was a :class:`DatetimeIndex`, :class:`TimedeltaIndex` or :class:`PeriodIndex` and the index held strings that parse to its values; such strings are distinct elements, as they already are for :meth:`Index.union`, :meth:`Index.intersection` and :meth:`Index.isin` (:issue:`58971`)
