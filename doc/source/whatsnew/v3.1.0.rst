...
- Fixed bug in :meth:`Series.isin` and :meth:`DataFrame.isin` returning incorrect results for a signed integer caller checked against a list-like of a different numeric dtype, where both sides were cast to ``float64`` and values above ``2**53`` could collide (:issue:`61676`)
