...
- Deprecated pinning the group key to the ``name`` attribute of the group passed to a user-defined function in :meth:`.SeriesGroupBy.apply`, :meth:`.SeriesGroupBy.transform`, :meth:`.DataFrameGroupBy.apply`, :meth:`.DataFrameGroupBy.transform`, :meth:`.DataFrameGroupBy.filter`, and the :class:`.Resampler` equivalents :meth:`.Resampler.apply`, :meth:`.Resampler.aggregate`, and :meth:`.Resampler.transform`. In a future version of pandas, the ``name`` attribute will no longer be set to the group key. When you need the key inside the function, iterate over the groupby object and combine the results yourself, e.g. ``pd.concat({key: func(group) for key, group in gb})`` when ``func`` returns a Series or DataFrame, or ``pd.Series({key: func(group) for key, group in gb})`` when it returns a scalar (:issue:`41090`)
