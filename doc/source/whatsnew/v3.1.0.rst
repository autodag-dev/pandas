...
- Fixed bug in :func:`read_excel` with a list-valued ``header`` and ``index_col`` where a sheet whose data columns were all consumed by ``index_col`` raised ``ValueError: Length of new names must be 1, got N`` instead of returning an empty :class:`DataFrame` as :func:`read_csv` does (:issue:`66372`)
