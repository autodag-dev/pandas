...
- Bug in :meth:`Series.convert_dtypes` and :meth:`DataFrame.convert_dtypes` raising ``OverflowError`` instead of retaining ``object`` dtype when given an integer outside the ``Int64``/``UInt64`` range (:issue:`66517`)
