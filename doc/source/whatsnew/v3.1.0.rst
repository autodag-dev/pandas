...
- :func:`read_sas` now raises an informative ``ValueError`` for a SAS7BDAT file that describes a column format for a column it never declared, instead of a bare ``IndexError`` from inside the reader, and for one that grows the reader's column metadata partway through a read, instead of silently doing so (:issue:`68066`)
