...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem broadcasting a 1-D value down rows instead of across the selected columns on an all-``datetime64`` or all-``timedelta64`` :class:`DataFrame` (e.g. ``df.iloc[[2, 0], ::-1] = value``), writing to the wrong cells or raising ``OutOfBoundsDatetime``; these assignments now behave as they do on a NumPy-backed :class:`DataFrame` (:issue:`68521`)
...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem transposing a 2-D value on an all-``datetime64`` or all-``timedelta64`` :class:`DataFrame` when both keys were list-like or boolean masks (e.g. ``df.iloc[[2, 0], [2, 1, 0]] = value``) (:issue:`68521`)
