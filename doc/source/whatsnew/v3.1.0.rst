...
- Deprecated ``inplace`` keyword for :meth:`DataFrame.rename`, :meth:`DataFrame.drop`, :meth:`DataFrame.dropna`, :meth:`DataFrame.drop_duplicates`, :meth:`DataFrame.sort_values`, :meth:`DataFrame.sort_index`, :meth:`DataFrame.eval`, :meth:`DataFrame.query`, :meth:`DataFrame.rename_axis` (and the ``Series`` equivalents). This keyword will be removed in a future version (:issue:`63207`); see also `PDEP-8 <https://pandas.pydata.org/pdeps/0008-inplace-methods-in-pandas.html>`_
...
- Bug in full-slice assignment (``arr[:] = value``, including ``df.loc[:, "a"] = other["b"]``) into a PyArrow-backed array, where the result shared memory with ``value``, so that later modifying ``value`` silently changed the assigned-to object too, in violation of Copy-on-Write (:issue:`67990`)
