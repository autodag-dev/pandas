...
- :meth:`DataFrame.filter` and :meth:`Series.filter` now accept a boolean mask, a callable returning a boolean mask, or an expression such as ``pd.col("a") > 1``, selecting the rows (or columns with ``axis=1``) where the mask is True. Missing values in the mask are treated as False by default, matching ``obj[mask]``; pass ``na=True`` to keep them or ``na="raise"`` to raise (:issue:`61317`)
...
Deprecations
~~~~~~~~~~~~
- Deprecated the ``items``, ``like``, and ``regex`` label-based usage of :meth:`DataFrame.filter` and :meth:`Series.filter`; in a future version ``filter`` will only accept a boolean mask. Use ``obj.loc`` with ``obj.columns.intersection(items)`` to keep a list of labels, and ``obj.filter(obj.columns.str.contains(pat), axis=1)`` in place of ``like`` or ``regex``. Passing boolean values when the filtered axis contains the labels ``True`` or ``False`` currently selects those labels and warns; in a future version the values will be treated as a boolean mask (:issue:`26642`, :issue:`61317`)
