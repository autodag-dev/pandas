...
- Bug in boolean-mask setitem, :meth:`Series.where`, :meth:`Series.mask`, and :meth:`Series.combine_first` aborting the Python process (``SIGABRT``) when applied to an :class:`ArrowDtype` ``null``-typed column due to a bug in PyArrow's ``replace_with_mask`` kernel for the ``null`` type (apache/arrow#47447, fixed in PyArrow 25.0.0); pandas now bypasses the kernel for ``null``-typed arrays (:issue:`66703`)
