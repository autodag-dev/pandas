...
- Bug in :meth:`DataFrame.clip` with ``inplace=True`` and an extension-dtype array or :class:`Index` bound silently writing wrong values, disagreeing with the same call without ``inplace``; a NumPy-dtype bound was unaffected (:issue:`68929`)
- Bug in :meth:`DataFrame.clip` with an extension-dtype array or :class:`Index` bound raising ``ValueError: Must specify axis=0 or 1`` when ``axis`` was not given, whereas a NumPy-dtype bound clipped along the columns (:issue:`68929`)
...
- Bug in :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.clip`, :meth:`Index.where`, :meth:`Index.putmask`, and ``.loc``/``.iloc`` setitem raising ``AttributeError: 'SparseDtype' object has no attribute 'itemsize'`` when the value was a :class:`SparseArray` and the target had an integer NumPy dtype (:issue:`68929`)
