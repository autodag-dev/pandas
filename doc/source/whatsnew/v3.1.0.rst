...
- Bug in :meth:`Series.cumsum` and :meth:`Series.cumprod` (and their :class:`DataFrame` counterparts) on sub-64-bit integer :class:`ArrowDtype` columns raising ``ArrowInvalid`` on overflow instead of widening to 64-bit like the NumPy and nullable backends; narrow integer columns now upcast to ``int64``/``uint64`` before accumulating (:issue:`66605`)
