...
- Bug in :meth:`DataFrame.to_string` and :meth:`DataFrame.__repr__` where ``min_rows=1`` was ignored and ``max_rows`` rows were shown instead of one, and where a terminal tall enough for exactly one row showed no rows at all; :class:`Series` already showed the single row (:issue:`64824`)
