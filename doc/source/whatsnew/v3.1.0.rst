...
- Performance improvement in :func:`read_csv` with ``engine="c"`` when parsing integer columns (:issue:`65347`, :issue:`66459`)
...
- Bug in :meth:`Timestamp.tz_localize`, :meth:`DatetimeIndex.tz_localize`, and :meth:`Series.dt.tz_localize` with a fixed-offset timezone where the shift to UTC landing exactly on the ``NaT`` sentinel (``INT64_MIN``) silently produced ``NaT`` or a wrapped value instead of raising :class:`OutOfBoundsDatetime` (:issue:`66550`)
