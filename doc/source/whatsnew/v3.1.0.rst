...
- Bug in :meth:`Index.join` with a :class:`MultiIndex` containing missing values in the joined level dropping or misaligning rows, which could raise during :class:`Series` arithmetic or return incorrect level-based reindexing results (:issue:`60908`)
