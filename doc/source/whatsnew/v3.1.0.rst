...
- :class:`DateOffset` now raises ``ValueError`` when a date component such as ``days``, ``weeks``, ``months`` or ``years`` is given a non-integer value, e.g. ``DateOffset(days=1.5)``. Applying the offset multiplies its ``relativedelta`` by ``n``, and that multiplication casts every component through ``int()``, so the fractional part was silently discarded and ``DateOffset(days=1.5)`` advanced by a whole day rather than by a day and a half. A fractional sub-daily value passed on its own, such as ``DateOffset(hours=1.5)``, is handled by ``timedelta`` and is unaffected (:issue:`55909`)
