...
- :meth:`DataFrame.agg` with a list of reductions now returns ``object`` for a column whose results cannot all be held exactly by one dtype, such as ``["sum", "mean"]`` on an ``int64`` column whose sum is not representable in ``float64``; previously such results were silently rounded (:issue:`65031`)
