...
- Bug in :meth:`DataFrame.to_html` where URLs were not properly escaped in the ``href`` attribute when ``render_links=True`` (:issue:`66080`)
- Bug in :meth:`DataFrame.to_html` where URLs were not properly escaped in the ``href`` attribute when ``render_links=True`` and ``escape=True`` (:issue:`66080`)
