...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` and ``header=None`` where ``dtype`` specifications keyed by label (or by position when ``names`` is not given) were not applied while parsing, losing the original text under string dtypes; the keys are now resolved against pyarrow's autogenerated column names (:issue:`21131`, :issue:`57666`)
