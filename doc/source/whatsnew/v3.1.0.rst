...
- Performance improvement in :meth:`.Rolling.skew`, :meth:`.Rolling.kurt`, and :meth:`.Expanding.skew`, which could be orders of magnitude slower on some inputs than on others of the same size (:issue:`68934`)
...
- Bug in :meth:`.Rolling.skew` and :meth:`.Rolling.kurt` returning ``NaN`` for all later windows once a window held no observations, e.g. due to ``NaN`` values, or held values spanning nearly the whole ``float64`` range (:issue:`68934`)
- Bug in :meth:`.Rolling.skew` and :meth:`.Rolling.kurt` returning ``NaN`` for low-variance windows (:issue:`62946`)
- Bug in :meth:`.Rolling.skew`, :meth:`.Rolling.kurt`, :meth:`.Expanding.skew`, and :meth:`.Expanding.kurt` losing most of their significant digits on data with a large offset shared by the whole series, such as prices or epoch timestamps (:issue:`68934`)
