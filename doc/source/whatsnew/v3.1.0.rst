...
- Fixed bug in :func:`read_csv` with ``engine="python"`` and :func:`read_html` where ``thousands`` and ``decimal`` preprocessing altered values before passing them to a ``converters`` callable (:issue:`39005`)
