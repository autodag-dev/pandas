...
- :meth:`DataFrame.to_sql` no longer raises a ``TypeError`` with ADBC connections when checking for a table in a schema that exists but has no tables (:issue:`59841`)
