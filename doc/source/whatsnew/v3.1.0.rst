...
Other enhancements
^^^^^^^^^^^^^^^^^^
- :class:`ExtensionScalarOpsMixin` no longer overwrites operator methods already defined on a subclass, allowing custom and fallback operators to coexist (:issue:`50767`)
