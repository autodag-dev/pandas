...
- :func:`read_csv` with ``sep=None`` and no explicit ``engine`` once again falls back to the ``python`` engine instead of raising ``TypeError``; explicitly passing ``engine="c"`` or ``engine="pyarrow"`` now raises ``ValueError`` for the unsupported option (:issue:`66639`)
