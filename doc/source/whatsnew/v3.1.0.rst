...
- Bug in ``isinstance(obj, DateOffset)`` and ``issubclass(cls, DateOffset)`` hanging indefinitely when called from a C extension, such as during ``pyarrow`` type inference, while a tracer such as ``coverage`` was active (:issue:`69154`)
