...
- Bug in :meth:`DataFrame.pivot_table` with ``margins=True`` raising ``KeyError`` or ``ValueError`` when ``aggfunc`` was a dict containing a list of functions for at least one column (:issue:`12210`)
