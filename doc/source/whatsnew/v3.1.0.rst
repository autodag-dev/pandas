...
- Bug in :meth:`Series.replace`, :meth:`Index.replace`, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.fillna` and item assignment (and their :class:`DataFrame` counterparts) on a PyArrow-backed integer dtype silently truncating a value with a fractional part, e.g. replacing with ``1.5`` stored ``1``; these now raise ``TypeError``, and :meth:`Index.where` and :meth:`Index.putmask` now upcast, both matching the nullable integer dtypes (:issue:`68638`)
