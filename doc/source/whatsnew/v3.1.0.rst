...
- Fixed bug in :meth:`DataFrame.resample` where resampling a :class:`DataFrame` with datetime that generates bin edges that are not valid in the index's timezone due to DST would not raise NonExistentTimeError and properly handle the DST gap (:issue:`62601`)
