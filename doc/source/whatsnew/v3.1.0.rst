...
.. _whatsnew_310.notable_bug_fixes.dataframe_rank_per_column:

:meth:`DataFrame.rank` ranks each column in its own dtype
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`DataFrame.rank` with ``axis=0`` cast the whole frame to one common dtype
before ranking, so its result could differ from :meth:`Series.rank` on the same
column. Each column is now ranked on its own and the two agree. Besides
retaining extension dtypes, this changes the ranks themselves for an ordered
:class:`Categorical` whose categories are not in the natural order of their
values, and for integers above ``2**53`` in a frame whose common dtype was
``float64`` (:issue:`52829`, :issue:`15420`).

.. code-block:: ipython

    In [1]: cat = pd.Categorical(
       ...:     ["b", "a", "c", "a"], categories=["c", "b", "a"], ordered=True
       ...: )

    In [2]: df = pd.DataFrame({"a": cat})

*Previous behavior*:

.. code-block:: ipython

    In [3]: df.rank()["a"].tolist()
    Out[3]: [3.0, 1.5, 4.0, 1.5]
...
- Bug in :meth:`DataFrame.rank` ranking an ordered :class:`Categorical` column by the natural ordering of its values rather than by the declared category order, unlike :meth:`Series.rank` (:issue:`15420`)
...
- Fixed bug in :meth:`DataFrame.rank` with ``axis=0`` giving tied ranks to distinct integer values above ``2**53`` when another column made ``float64`` the frame's common dtype (:issue:`69136`)
...
- Fixed bug in :meth:`DataFrame.rank` with ``axis=0`` returning ``float64`` columns for extension dtypes such as nullable integer or :class:`ArrowDtype`, rather than the dtype :meth:`Series.rank` returns (:issue:`52829`)
