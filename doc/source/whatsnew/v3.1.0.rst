...
.. _whatsnew_310.notable_bug_fixes.ewm_nan_adjust_false:

``ewm(adjust=False)`` weights a new observation by the current step only
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`Series.ewm` / :meth:`DataFrame.ewm` with ``adjust=False`` and
``ignore_na=False`` used a convex combination over the *accumulated* decay
when the decay was ``com=1`` (equivalently ``span=3``, ``alpha=0.5``, or
``halflife=1``), including whenever ``times`` was supplied. A ``NaN`` gap
therefore gave the new observation the weight forgotten since the last
non-null value, so ``Series([1, np.nan, 5]).ewm(com=1, adjust=False).mean()``
returned ``4.0`` instead of the documented
:math:`((1-\alpha)^2 \cdot 1 + \alpha \cdot 5) / ((1-\alpha)^2 + \alpha) = 11/3`.

This was a regression in 3.0.0 on the equally spaced (no-``times``) path;
2.3.x returned ``11/3``. The 3.0.0 ``times`` spelling of the same call
returned ``4.0`` as well and now matches. Only :meth:`ExponentialMovingWindow.mean`
is affected. ``adjust=True``, ``ignore_na=True``, irregular ``times``
without missing values, and ``std`` / ``var`` / ``cov`` / ``corr`` are
unchanged (:issue:`31178`, :issue:`66523`).
...
- Deprecated ``inplace`` keyword for :meth:`DataFrame.rename`, :meth:`DataFrame.drop`, :meth:`DataFrame.dropna`, :meth:`DataFrame.drop_duplicates`, :meth:`DataFrame.sort_values`, :meth:`DataFrame.sort_index`, :meth:`DataFrame.eval`, :meth:`DataFrame.query` (and the ``Series`` equivalents). This keyword will be removed in a future version (:issue:`63207`); see also `PDEP-8 <https://pandas.pydata.org/pdeps/0008-inplace-methods-in-pandas.html>`_
...
- Bug in :meth:`DataFrame.ewm` and :meth:`Series.ewm` with ``adjust=False`` and ``ignore_na=False`` returning a convex combination over a ``NaN`` gap when the decay was ``com=1`` (equivalently ``span=3``, ``alpha=0.5``, or ``halflife=1``), including when equally spaced ``times`` were supplied; the new observation is now weighted by the current step only and the result matches the documented ``ignore_na=False`` recursion (:issue:`31178`, :issue:`66523`)
- Bug in :meth:`DataFrame.ewm` and :meth:`Series.ewm` with ``engine="numba"`` raising on a length-1 object because the interval tuple was empty (:issue:`66523`)
...
- Bug in :meth:`OnlineExponentialMovingWindow.mean` raising ``IndexError`` on a length-1 :class:`Series` (:issue:`66523`)
