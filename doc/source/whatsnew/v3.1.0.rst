...
.. _whatsnew_310.enhancements.select:

New method ``DataFrame.select``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`DataFrame.select` returns a new DataFrame containing the specified
columns. In addition to existing column labels, columns
can be computed via :func:`pandas.col` expressions, or callables passed as
keyword arguments, with the keyword naming the resulting column. As with
:meth:`DataFrame.assign`, later arguments can refer to columns computed
...
.. _whatsnew_310.enhancements.filter_mask:

Filtering with a boolean mask in ``DataFrame.filter`` and ``Series.filter``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

:meth:`DataFrame.filter` and :meth:`Series.filter` now accept a boolean mask through the new
``cond`` keyword, keeping the rows (or columns with ``axis=1``) where the mask is True.
``cond`` may also be a callable returning a boolean mask or an expression created with
:func:`pandas.col`; these two forms may also be passed positionally, which is convenient in
method chains (:issue:`61317`).

.. code-block:: ipython

    In [1]: df = pd.DataFrame({"a": [1, 2, 3], "b": [4, 5, 6]})

    In [2]: df.filter(cond=df["a"] > 1)
    Out[2]:
       a  b
    1  2  5
    2  3  6

    In [3]: df.filter(pd.col("a") > 1)
    Out[3]:
       a  b
    1  2  5
    2  3  6

    In [4]: df.filter(lambda df: df["b"] < 6)
    Out[4]:
       a  b
    0  1  4
    1  2  5

A :class:`Series` mask is aligned with the filtered axis; any other mask must have the same
length as that axis. Missing values in the mask are treated as False by default, matching
``obj[mask]`` for a mask with nullable boolean dtype. Pass ``na=True`` to keep the
corresponding entries instead, or ``na="raise"`` to raise.

.. code-block:: ipython

    In [5]: mask = pd.array([True, None, False], dtype="boolean")

    In [6]: df.filter(cond=mask)
    Out[6]:
       a  b
    0  1  4

    In [7]: df.filter(cond=mask, na=True)
    Out[7]:
       a  b
    0  1  4
    1  2  5

Any other value passed positionally continues to select labels, as before. Since a list-like
of booleans passed this way is likely intended as a mask, it now issues a ``UserWarning``;
pass ``items`` to select labels or ``cond`` to filter with a mask. Selecting labels with
``items``, ``like``, or ``regex`` will be deprecated in a future version.
...
number of CPUs available to the process, capped at 6. Set it to ``1`` to
...
- Added :meth:`ExtensionDtype._get_plot_converter` allowing :class:`ExtensionDtype` to register their own :class:`matplotlib.units.ConversionInterface` to support plotting for non-numeric :class:`ExtensionArray` (:issue:`64300`)
...
- Vertical bar plots with a regular-frequency :class:`DatetimeIndex` or :class:`PeriodIndex` now format the x-axis with the same dynamic date tick labeling as line plots, instead of labeling every bar with the full string representation of its timestamp. The tick labels of such plots are no longer rotated by default, matching line plots; pass ``rot`` to override. Bar plots over a frequency the date ticks cannot be aligned to — a multiplied business-day, monthly, quarterly or annual frequency such as ``"3B"`` or ``"2ME"`` — keep the previous labeling (:issue:`1918`)
...
.. _whatsnew_310.notable_bug_fixes.read_csv_categorical:

:func:`read_csv` with ``dtype="category"`` infers numeric and boolean categories
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. _whatsnew_310.api_breaking.datetimelike_unit_numeric:

Datetime/timedelta resolution for numeric input
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In pandas 3.0, the default ``unit`` when creating ``datetime64`` or
``timedelta64`` data was changed from nanoseconds to microseconds for many
cases (e.g. constructing from strings or stdlib ``datetime`` objects). But for
numeric input, it was changed to take the resolution used to interpret the
input value as the resulting resolution. See
:ref:`whatsnew_300.api_breaking.datetime_resolution_inference` in the 3.0
release notes for more details.

This means that, for example, ``pd.to_timedelta([1], unit="s")`` now had ``s``
resolution instead of ``ns``. An unintended side effect of this change is that
doing arithmetic with such ``timedelta64[s]`` data, which preserves the ``s``
resolution, could lead to unexpected (rounded) results (see :issue:`65608`).

To ensure a more consistent use of resolution, construction with numeric input
will now also default to microseconds (or nanoseconds if the precision of the
input requires it).

Specifically, :func:`to_datetime`, :func:`to_timedelta`, :class:`Timestamp`,
and :class:`Timedelta` constructors will still use the ``unit`` argument to
convert numeric input, but no longer use that resolution for the resulting
``dtype``. For example, the following cases now all default to microseconds:

.. code-block:: pycon

   >>> print(pd.to_datetime([1, 2], unit="s").dtype)
   datetime64[us]
   >>> print(pd.to_timedelta([1, 2], unit="s").dtype)
   timedelta64[us]
   >>> pd.Timestamp(1, unit="s").unit
   'us'
   >>> pd.Timedelta(1, unit="s").unit
   'us'

Compared to the behaviour in pandas 2.x, this means that those cases now give
microsecond resolution instead of nanoseconds. Except for the ``pd.Timestamp(1,
unit="s")`` case, which already used the input ``unit`` before pandas 3.0,
and thus now gives microsecond instead of second resolution.

If you actually need the specific resolution from the input data, use the
:meth:`~Timestamp.as_unit` (or :meth:`Series.dt.as_unit`) method after
construction to convert to the desired resolution.
...
- :func:`read_csv` and :func:`read_table` now raise ``ValueError`` when ``delimiter=None`` is passed together with ``sep``, as they already did for any other pair of both; previously the ``delimiter`` was silently ignored. :func:`read_clipboard` always passes ``sep`` on, so it now raises for ``delimiter=None`` as it already did for any other ``delimiter`` (:issue:`47024`)
...
- :meth:`DataFrame.select_dtypes` now raises ``TypeError`` when a Python class that does not name a dtype, such as ``decimal.Decimal``, ``datetime.date`` or :class:`Timestamp`, is passed to ``include`` or ``exclude``. Previously such a class selected every object-dtype column, not only the columns holding that class; pass ``object`` to keep that behavior (:issue:`68443`)
...
- :meth:`DataFrame.select_dtypes` now raises ``ValueError`` for an :class:`IntervalDtype` or :class:`CategoricalDtype` spec that gives some of its attributes but not all: ``pd.IntervalDtype("int64")`` or ``"interval[int64]"`` with no ``closed``, ``pd.IntervalDtype(closed="left")`` with no subtype, ``"interval[datetime64]"`` whose subtype names no resolution, or ``pd.CategoricalDtype(ordered=True)`` with no categories. No column has such a dtype, and ``"interval[int64]"`` previously selected every interval column; an interval subtype names a resolution specifically, unlike the top-level unitless ``"datetime64"``. A bare ``pd.IntervalDtype()`` or ``pd.CategoricalDtype()`` still selects every interval or categorical column, as the class and the ``"interval"``/``"category"`` strings do (:issue:`40234`)
- :meth:`DataFrame.select_dtypes` now raises ``ValueError`` when an interval spec's subtype names a resolution no column can have, such as ``"interval[datetime64[10s]]"`` or ``pd.IntervalDtype(np.dtype("M8[Y]"))``, instead of selecting no columns (:issue:`40234`)
...
- :meth:`DataFrame.select_dtypes` now treats the string specs ``"datetime64[ns]"`` and ``"timedelta64[ns]"`` as naming the nanosecond resolution specifically, so they select only ``ns`` columns; previously they selected every datetime64/timedelta64 column regardless of resolution. Pass the unitless ``"datetime64"``/``"timedelta64"`` to keep that behavior. A spec giving a multiple of a unit, such as ``"timedelta64[2ns]"``, now raises instead of selecting every column (:issue:`40234`)
...
- :meth:`HDFStore.select`, :meth:`HDFStore.select_as_coordinates`, :meth:`HDFStore.select_as_multiple`, :meth:`HDFStore.remove` and :func:`read_hdf` now raise ``TypeError`` for an ordering comparison (``<``, ``<=``, ``>``, ``>=``) in a ``where`` query against an unordered categorical column or index, matching the comparison on the same data in memory. Previously the query ranked the rows by the arbitrary order the categories happened to be stored in. Write the column with ``ordered=True`` to keep querying it by order (:issue:`68040`)
- :meth:`Series.shift` now raises ``ValueError`` when ``suffix`` is passed together with an integer ``periods``. Previously the argument was silently ignored (:issue:`54806`)
...
- ``numexpr`` 2.14.1 is no longer used to accelerate arithmetic and comparisons; some builds of that release intermittently and silently returned incorrect results, such as an all-zero column, for operations on large objects. pandas now falls back to NumPy, warning the first time an operation would have used ``numexpr``, and :func:`eval`, :meth:`DataFrame.eval` and :meth:`DataFrame.query` raise ``ImportError`` when passed an explicit ``engine="numexpr"``; install ``numexpr`` 2.14.2 or newer to restore the acceleration. HDF5 ``where=`` queries are evaluated by PyTables, which imports ``numexpr`` itself, and are neither covered nor warned about (:issue:`65408`)
-
...
.. _deprecation-inplace:

Deprecated ``inplace`` keyword for various methods
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

PDEP-8: https://pandas.pydata.org/pdeps/0008-inplace-methods-in-pandas.html

The ``inplace`` keyword is deprecated (and will eventually be removed) for all
methods that do not actually work "inplace".

The ``inplace`` keyword has been used in various methods throughout pandas, but
often, it is essentially syntactic sugar for reassigning the result to the
calling DataFrame or Series, because it never modifies the underlying values
directly. Additionally, with the introduction of Copy-on-Write in pandas 3.0,
users don't need the ``inplace`` keyword to avoid a copy of the data.

For those reasons, the ``inplace`` keyword is deprecated for those methods
which do not work inplace. In practice, these are the DataFrame and Series
methods :meth:`~DataFrame.drop`, :meth:`~DataFrame.rename`,
:meth:`~DataFrame.rename_axis`, :meth:`~DataFrame.reset_index`,
:meth:`~DataFrame.set_index`, :meth:`~DataFrame.dropna`,
:meth:`~DataFrame.drop_duplicates`, :meth:`~DataFrame.sort_values`,
:meth:`~DataFrame.sort_index`, :meth:`~DataFrame.eval`,
:meth:`~DataFrame.query`.

If you have code that uses a deprecated instance of ``inplace``, such as:
...
Then you can update that call to re-assign the result:
...
In practice, this is also what happened under the hood (i.e. it is not less efficient),
and this is more friendly to method chaining.

Other methods, such as :meth:`~DataFrame.fillna` and :meth:`~DataFrame.replace`,
which can actually update the underlying values, keep the keyword (with a
default of ``False``, and since pandas 3.0 those also return the updated object).

For more background about the change, see the motivation section in `PDEP-8 <https://pandas.pydata.org/pdeps/0008-inplace-methods-in-pandas.html>`__.

Other deprecations
^^^^^^^^^^^^^^^^^^

- Added ``check_freq`` keyword to :func:`testing.assert_index_equal` with a deprecated default: currently a ``freq`` mismatch on a :class:`DatetimeIndex` or :class:`TimedeltaIndex` only warns; in a future version ``check_freq=True`` will be the default and mismatches will raise. The same deprecation applies in :func:`testing.assert_frame_equal` and :func:`testing.assert_series_equal` to freqs that were previously never checked: those of :class:`DatetimeIndex`/:class:`TimedeltaIndex` columns, :class:`MultiIndex` levels, and :class:`Categorical` categories. Pass ``check_freq`` explicitly to silence the warning (:issue:`51920`, :issue:`66761`)
...
- Deprecated :meth:`DataFrame.select_dtypes` matching the default ``str`` dtype when ``object`` is passed to ``exclude``, mirroring the existing deprecation for ``include``. Pass ``"str"`` to ``exclude`` to drop those columns as well, or to ``include`` to keep them (:issue:`68454`)
...
- Deprecated arithmetic, comparison, and logical operations between pandas objects (:class:`DataFrame`, :class:`Series`, :class:`Index`, and pandas-implemented :class:`ExtensionArray` subclasses) and list-likes other than ``list``, ``np.ndarray``, :class:`ExtensionArray`, :class:`Index`, :class:`Series`, :class:`DataFrame`. For e.g. ``tuple`` or ``range``, explicitly cast these to a supported object instead. In a future version, these will be treated as scalar-like for pointwise operation (:issue:`62423`)
...
- Deprecated indexing with ``[]`` on the result of :meth:`.GroupBy.nth` (e.g. ``gb.nth[0, 1]``); call it instead (e.g. ``gb.nth([0, 1])``) (:issue:`66346`)
...
- Deprecated passing ``None`` inside the list-like ``include`` or ``exclude`` argument of :meth:`DataFrame.select_dtypes`, which silently selects float64 columns. Pass ``"float64"`` explicitly instead (:issue:`28943`)
...
- Deprecated the ``Groupby`` spellings in ``pandas.api.typing`` (``DatetimeIndexResamplerGroupby``, ``ExpandingGroupby``, ``ExponentialMovingWindowGroupby``, ``PeriodIndexResamplerGroupby``, ``RollingGroupby``, ``TimedeltaIndexResamplerGroupby``); the classes are renamed to the ``GroupBy`` spelling used by :class:`.DataFrameGroupBy` and :class:`.SeriesGroupBy`, e.g. ``RollingGroupBy``, which also changes their ``repr`` (:issue:`49578`)
...
- Deprecated the ``future.infer_string`` option. Inferring str dtype for string data has been the default since pandas 3.0, and in pandas 4.0 the option will be removed and it will no longer be possible to disable this inference (:issue:`68436`)
...
- Deprecated the ``lsuffix`` and ``rsuffix`` keywords in :meth:`DataFrame.join` in favor of ``suffixes``, consistent with :func:`merge` and :meth:`DataFrame.merge` (:issue:`46046`)
- Deprecated the ``mode.chained_assignment`` option. It has had no effect since pandas 3.0 removed ``SettingWithCopyWarning``; chained assignment now emits a :class:`ChainedAssignmentError` instead, which can be silenced or turned into an error with ``warnings.filterwarnings(..., category=ChainedAssignmentError)`` (:issue:`67987`)
...
- Deprecated passing integers to :class:`Period`, :class:`PeriodIndex`, :func:`period_range`, :func:`period_array`, and :meth:`PeriodArray._from_sequence`. In a future version, integers will be treated as period ordinals instead of calendar years. To get the future behavior now, use ``Period(ordinal=...)`` or :meth:`PeriodIndex.from_ordinals`; to keep the current behavior, pass strings, e.g. ``Period(str(value), freq=...)`` or ``data.astype(str)`` (:issue:`64227`)
...
- Performance improvement in :func:`read_csv` with ``engine="c"`` across the board, in tokenizing and in the machinery shared by all column types; large uncompressed local files are additionally read in parallel across multiple CPU cores by default, see :ref:`above <whatsnew_310.enhancements.read_csv>` (:issue:`64347`, :issue:`64515`, :issue:`65350`, :issue:`66271`, :issue:`66272`, :issue:`66273`, :issue:`66274`, :issue:`66276`, :issue:`66798`, :issue:`68314`)
- Performance improvement in :func:`read_csv` with ``engine="c"`` for integer and float columns, and when an extension dtype such as ``dtype="Int64"``, ``dtype="boolean"`` or ``dtype="int64[pyarrow]"`` is requested (:issue:`65347`, :issue:`65767`, :issue:`66239`, :issue:`66279`, :issue:`66457`, :issue:`66459`, :issue:`66487`, :issue:`68380`)
- Performance improvement in :func:`read_csv` with ``engine="c"`` for string columns, in particular under ``future.infer_string`` or ``dtype_backend="pyarrow"``; reads issued concurrently from several threads also scale better (:issue:`65283`, :issue:`66277`, :issue:`66619`, :issue:`66756`, :issue:`68379`)
...
- Performance improvement in :func:`testing.assert_frame_equal`, :func:`testing.assert_series_equal` and :func:`testing.assert_index_equal` when comparing exactly-equal integer and floating values above ``2**53`` without checking dtype, which fell back to an element-by-element comparison (:issue:`68568`)
...
- Performance improvement in :meth:`DataFrame.duplicated` and :meth:`DataFrame.drop_duplicates` with multiple columns when one of them has :class:`PeriodDtype` (:issue:`69025`)
...
- Bug in :meth:`Series.replace` and :meth:`DataFrame.replace` with :class:`CategoricalDtype` silently discarding a regular-expression replacement (:issue:`38447`)
-
...
- Bug in :class:`DatetimeIndex` and :class:`TimedeltaIndex` constructors failing to raise ``ValueError`` when passed a 0-dimensional ``ndarray`` input (:issue:`69411`)
...
- Bug in :class:`Series`, :class:`DataFrame`, :class:`Index`, :class:`Timedelta`, :func:`array`, :func:`to_datetime` and :func:`to_timedelta` construction from ``datetime64``/``timedelta64`` input whose unit carries a multiplier (e.g. ``np.array([1, 2], dtype="m8[10s]")``): every timedelta64 value came back divided by the multiplier and datetime64 input raised a bare ``AssertionError``. Such input, and a ``dtype`` argument such as ``"m8[10s]"``, now raise ``ValueError`` (:issue:`25611`)
- Bug in :class:`Timestamp` constructor where a timezone-aware ``datetime`` near the implementation bounds raised ``OverflowError`` instead of ``OutOfBoundsDatetime`` when shifted to UTC (:issue:`66510`)
...
- Bug in :class:`Timestamp` constructor, :func:`to_datetime`, and :func:`read_csv` with ``parse_dates`` where strings with a ``"GMT"`` timezone name raised ``ValueError`` on systems whose local timezone is named ``GMT`` (e.g. ``Europe/London``, ``Africa/Abidjan``) instead of parsing to UTC as they do elsewhere (:issue:`66827`)
- Bug in :class:`Timestamp` constructor, :meth:`Timestamp.replace`, :func:`to_datetime`, and :func:`read_csv` with ``parse_dates`` where a datetime landing on the ``NaT`` sentinel, either directly or after the shift to UTC, silently became ``NaT`` instead of raising ``OutOfBoundsDatetime`` (:issue:`66510`)
...
- Bug in :func:`to_datetime` and :class:`Timestamp` with ``unit="Y"`` or ``unit="M"`` and an infinite ``float``, raising ``ValueError`` instead of ``OutOfBoundsDatetime`` (:issue:`68640`)
...
- Bug in :func:`to_datetime` and :func:`to_timedelta` where ``uint64`` values greater than ``int64`` max silently overflowed instead of raising ``OutOfBoundsDatetime`` or ``OutOfBoundsTimedelta`` (:issue:`60677`)
...
- Bug in :func:`to_datetime` with ``unit`` and a ``float`` too large to represent, returning ``NaT`` or raising ``OverflowError`` instead of ``OutOfBoundsDatetime`` (:issue:`68640`)
- Bug in :func:`to_datetime` with string input silently ignoring ``dayfirst`` and ``yearfirst`` when ``unit`` was also passed (:issue:`63472`)
- Bug in :func:`to_datetime` with the default ``cache=True`` returning ``category`` or ``object`` dtype instead of ``datetime64`` for categorical input with enough repeated values for the cache to be used (:issue:`28629`)
...
- Bug in :meth:`DatetimeIndex.round`, :meth:`TimedeltaIndex.round`, :meth:`Series.dt.round`, :meth:`Timestamp.round`, and :meth:`Timedelta.round` (and their ``floor`` and ``ceil`` counterparts) on non-nanosecond data silently rounding to the wrong frequency when ``freq`` was neither a whole multiple of the object's resolution nor an exact divisor of it, e.g. ``freq="2500ms"`` on second-resolution data rounding to 2 seconds; these now raise ``ValueError`` (:issue:`67978`)
- Bug in :meth:`DatetimeIndex.tz_localize` dropping the ``freq`` attribute for calendar-based frequencies such as ``"D"`` or ``"MS"``, which localization leaves valid (:issue:`36575`)
- Bug in :meth:`DatetimeIndex.tz_localize`, :meth:`Series.dt.tz_localize`, and the :class:`Timestamp` constructor with a timezone that observes DST, where a wall time whose shift to UTC lands exactly on the ``NaT`` sentinel read out of bounds and reported a spurious "nonexistent time" ``ValueError`` instead of raising ``OutOfBoundsDatetime`` (:issue:`66550`)
...
- Bug in ``np.logical_and``, ``np.logical_or``, ``np.logical_xor`` and ``np.logical_not`` returning a result for datetime-like data -- including the timezone-aware, :class:`PeriodDtype`, :class:`ArrowDtype` and :class:`SparseDtype` spellings, and the corresponding scalars -- instead of raising ``TypeError``; this now also rejects a ``NaT`` scalar against an :class:`ArrowDtype` operand, which ``&`` accepts as a null (:issue:`68524`)
...
- Bug in adding a :class:`DateOffset` with both a month- or year-based component and a timedelta component (e.g. ``DateOffset(months=1, days=-31)``) to a :class:`DatetimeIndex` or :class:`Series` raising ``OutOfBoundsDatetime`` for a representable result, where the equivalent :class:`Timestamp` operation succeeds (:issue:`66549`)
...
- Bug in adding a business-day-based offset (e.g. :class:`BMonthBegin`, :class:`BQuarterEnd`, :class:`BYearBegin`) to a :class:`Timestamp` outside the range of ``datetime.datetime``, e.g. ``Timestamp(np.datetime64("-19999-02-01"))``, returning a date computed from the wrong weekday (:issue:`53125`)
...
- Bug in adding a month-, quarter-, year- or semi-month-based offset (e.g. :class:`MonthEnd`, :class:`QuarterEnd`, :class:`YearEnd`, :class:`SemiMonthEnd`) to a :class:`DatetimeIndex` or :class:`Series` where an out-of-bounds result raised a bare ``OverflowError`` naming a pandas C source file, instead of the ``OutOfBoundsDatetime`` naming the offending date that the equivalent :class:`Timestamp` operation raises (:issue:`66549`)
- Bug in adding a quarter-, half-year- or year-based offset (e.g. :class:`QuarterEnd`, :class:`HalfYearEnd`, :class:`YearEnd`) with ``n`` near ``2**63`` to a :class:`Timestamp` raising ``OutOfBoundsDatetime`` naming a negative year for a positive shift (:issue:`66549`)
- Bug in adding a quarter-, half-year- or year-based offset (e.g. :class:`QuarterEnd`, :class:`HalfYearEnd`, :class:`YearEnd`) with a very large ``n`` to a :class:`DatetimeIndex` or :class:`Series` silently shifting by a wrapped number of months, e.g. ``YearEnd(2**62)`` behaving like ``n=0``, instead of raising ``OutOfBoundsDatetime`` as the equivalent :class:`Timestamp` operation does (:issue:`66549`)
- Bug in adding an unsupported object to a :class:`.DateOffset` raising ``TypeError: Argument 'other' has incorrect type (expected datetime.datetime, ...)`` instead of the standard ``unsupported operand type(s)`` message (:issue:`36590`)
...
- Bug in adding or subtracting a :class:`Timedelta` and a :class:`Timestamp` where a result landing exactly on the ``NaT`` sentinel, e.g. ``Timestamp.min + Timedelta(-1, "ns")``, returned ``NaT`` instead of raising ``OutOfBoundsDatetime`` as it already did one step further out (:issue:`66549`)
- Bug in adding or subtracting a :class:`Timedelta` and a ``datetime64`` :class:`numpy.ndarray` where the promotion to the finer of the two resolutions, or the addition itself, silently wrapped to an unrelated date, and a result landing exactly on the ``NaT`` sentinel came back as a missing value, instead of raising ``OutOfBoundsDatetime`` or ``OutOfBoundsTimedelta`` as the equivalent :class:`Timestamp` operation already did (:issue:`66552`)
- Bug in adding or subtracting a ``timedelta64`` NumPy array and a timezone-naive :class:`Timestamp` where the promotion to the finer of the two resolutions, or the addition itself, silently wrapped to an unrelated date instead of raising ``OutOfBoundsDatetime``, unlike the timezone-aware and scalar equivalents (:issue:`66552`)
- Bug in adding or subtracting a boolean array and ``datetime64`` or ``timedelta64`` data silently treating ``True`` as one unit of the data's resolution instead of raising ``TypeError`` as integer data does; ``datetime64`` with a :class:`BooleanDtype` operand instead raised ``ValueError: Could not convert object to NumPy datetime`` (:issue:`68452`)
...
- Bug in logical operations (``&``, ``|``, ``^``) between a pandas object and any datetime-like operand -- ``datetime64`` (including timezone-aware), ``timedelta64``, :class:`PeriodDtype`, any temporal :class:`ArrowDtype`, their :class:`SparseDtype` spellings, and a :class:`Categorical` of any of them -- silently treating every value, ``NaT`` included, as ``True`` instead of raising ``TypeError`` (:issue:`68452`)
- Bug in multiplying a :class:`Timedelta` by a bool-dtype array, :class:`Index`, or :class:`Series` silently treating ``True`` as 1 and ``False`` as 0 instead of raising ``TypeError`` as it already did for a scalar bool (:issue:`62316`)
...
- Bug in subtracting two :class:`Timestamp` objects whose difference lands exactly on the ``NaT`` sentinel raising ``AssertionError``, or with ``python -O`` returning a corrupt :class:`Timedelta` for which :func:`isna` was ``False``, instead of raising ``OutOfBoundsDatetime`` (:issue:`66552`)
...
- Bug in :class:`Series`, :class:`DataFrame`, and :class:`TimedeltaIndex` construction with a ``timedelta64[unit]`` dtype, where numbers mixed with timedelta-like objects were interpreted as nanoseconds instead of in the dtype's unit, e.g. ``pd.Series([pd.Timedelta(1, "s"), 2], dtype="timedelta64[s]")`` (:issue:`68639`)
...
- Bug in :class:`Timedelta` constructor where keyword arguments given as ``float`` (e.g. ``Timedelta(days=365000, seconds=1.5)``) still raised ``OutOfBoundsTimedelta`` for durations that are exactly representable at a coarser resolution, where the equivalent integer keywords succeeded, and where ``Timedelta(days=1e11)`` disagreed with ``Timedelta(days=10**11)`` (:issue:`46587`)
- Bug in :class:`Timedelta` constructor where keyword arguments naming a duration exactly at the ``int64`` minimum, e.g. ``Timedelta(seconds=-2**63)``, silently returned ``NaT`` instead of raising ``OutOfBoundsTimedelta`` (:issue:`46587`)
- Bug in :class:`Timedelta` constructor with a ``NaN`` keyword argument, e.g. ``Timedelta(days=np.nan)``, raising ``ValueError`` instead of returning ``NaT`` as the equivalent positional call ``Timedelta(np.nan, unit="D")`` does; cancelling infinities such as ``Timedelta(days=np.inf, seconds=-np.inf)`` now raise ``OutOfBoundsTimedelta`` rather than ``ValueError`` (:issue:`66823`)
- Bug in :class:`Timedelta` constructor with keyword arguments raising a bare ``OverflowError`` for infinite float input, e.g. ``Timedelta(days=np.inf)``, instead of ``OutOfBoundsTimedelta`` (:issue:`66823`)
- Bug in :func:`to_timedelta` with ``errors="coerce"`` raising ``OutOfBoundsTimedelta`` instead of returning ``NaT`` for out-of-bounds or infinite values in float-dtype input, e.g. ``to_timedelta(np.array([1.5, 1e30]), unit="s", errors="coerce")``; this also affected :func:`to_datetime` assembling from a :class:`DataFrame` with a float time-unit column (:issue:`66823`)
- Bug in :func:`to_timedelta` with ``errors="coerce"`` where an out-of-bounds value coerced to ``NaT`` still determined the resolution of the result, which could push an unrelated, representable value out of bounds and silently coerce it to ``NaT`` as well, e.g. ``to_timedelta([10**19, np.timedelta64(10**12, "s")], errors="coerce")``; this also affected :func:`to_datetime` assembling from a :class:`DataFrame`, whose result resolution was likewise set by a coerced-away time-unit value (:issue:`65150`)
- Bug in :func:`to_timedelta` with a :class:`Day` offset too large to represent, e.g. ``to_timedelta(Day(10**17))``, raising a bare ``OverflowError`` instead of ``OutOfBoundsTimedelta``, so ``errors="coerce"`` propagated the exception instead of returning ``NaT`` (:issue:`64240`)
- Bug in :func:`to_timedelta`, :class:`TimedeltaIndex`, and :class:`Series`/:class:`DataFrame` construction or ``astype`` on object-dtype input, where a ``numpy.timedelta64`` ``NaT`` whose unit differed from the array's inferred resolution raised ``OutOfBoundsTimedelta`` instead of being preserved as ``NaT``, e.g. ``to_timedelta([np.timedelta64("NaT", "D")])`` (:issue:`63018`)
...
- Bug in :meth:`Series.astype`, :meth:`DataFrame.astype`, :meth:`Index.astype`, and :class:`Series`/:class:`DataFrame` construction from existing object-dtype data, with a ``timedelta64`` dtype: an object array whose non-null values are all numbers was read as nanoseconds rather than in the dtype's unit, so ``Series([2, 3], dtype=object).astype("m8[s]")`` came back all zeros (:issue:`68688`)
- Bug in :meth:`Series.cumsum` and :meth:`DataFrame.cumsum` on ``timedelta64`` data where a running total that left the representable range silently wrapped to an unrelated value instead of raising ``OutOfBoundsTimedelta`` (:issue:`66551`)
...
- Bug in :meth:`Series.sum` and :meth:`DataFrame.sum` on an overflowing ``timedelta64`` object where :meth:`Series.sum` raised a plain ``ValueError`` and :meth:`DataFrame.sum` silently returned a saturated result instead of raising ``OutOfBoundsTimedelta`` (:issue:`43178`)
...
- Bug in adding or subtracting a :class:`Timedelta` and a ``timedelta64`` :class:`numpy.ndarray` where an out-of-bounds result silently wrapped, and a result landing exactly on the ``NaT`` sentinel came back as a missing value, instead of raising ``OutOfBoundsTimedelta`` as the equivalent :class:`Timedelta` and :class:`TimedeltaIndex` operations already did (:issue:`66552`)
- Bug in adding or subtracting two :class:`Timedelta` objects where a result landing exactly on the ``NaT`` sentinel returned ``NaT``, e.g. ``Timedelta.min - Timedelta(1, "ns")``, instead of raising ``OutOfBoundsTimedelta`` as it already did one step further out (:issue:`66552`)
- Bug in dividing a :class:`Timedelta` by a float :class:`numpy.ndarray` where a quotient outside the ``int64`` range silently saturated instead of raising ``OutOfBoundsTimedelta`` as the equivalent :class:`TimedeltaIndex` operation already did (:issue:`66552`)
...
- Bug in multiplying a :class:`Series` of ``timedelta64`` by an integer or float, or dividing it by a float, where an overflowing result silently wrapped or saturated instead of raising ``OutOfBoundsTimedelta``; multiplying a non-zero value by ``inf`` now raises ``OutOfBoundsTimedelta`` instead of returning ``NaT`` (:issue:`43178`)
...
- Bug in multiplying a :class:`Timedelta` by an integer or float :class:`numpy.ndarray` where an out-of-bounds product silently wrapped or saturated, and one landing exactly on the ``NaT`` sentinel came back as a missing value, instead of raising ``OutOfBoundsTimedelta`` as the equivalent :class:`TimedeltaIndex` operation already did (:issue:`66552`)
- Bug in multiplying a zero :class:`Timedelta` by ``inf`` raising ``ValueError: cannot convert float NaN to integer`` instead of returning ``NaT`` as the equivalent :class:`TimedeltaIndex` and ``timedelta64`` :class:`Series` operations already did (:issue:`68392`)
- Bug in multiplying or dividing a :class:`Timedelta` by a numeric scalar where a result landing exactly on the ``NaT`` sentinel raised a message-less ``AssertionError``, or under ``python -O`` returned a :class:`Timedelta` that was not ``NaT`` but read back as ``NaT`` once stored; these now raise ``OutOfBoundsTimedelta`` (:issue:`66551`)
- Bug in multiplying or dividing a :class:`Timedelta` by a numeric scalar where an out-of-bounds result raised a bare ``OverflowError`` instead of ``OutOfBoundsTimedelta`` as the equivalent :class:`TimedeltaIndex` and ``timedelta64`` :class:`Series` operations already did (:issue:`68393`)
...
- Bug in :meth:`DatetimeIndex.tz_localize`, :meth:`Series.dt.tz_localize`, and :meth:`Timestamp.tz_localize` with a timedelta ``nonexistent`` shift silently wrapping and returning an incorrect timestamp instead of raising ``OutOfBoundsDatetime`` when the shift leaves the representable range (:issue:`66697`)
...
- Bug in timezone-aware :class:`DatetimeIndex` and :class:`Series` where a UTC instant near :attr:`Timestamp.min` or :attr:`Timestamp.max` whose local wall time is outside the representable range silently reported a wall time roughly 585 years off, or ``NaT``, instead of raising ``OutOfBoundsDatetime``; this affected ``tz_localize(None)``, field accessors such as ``.year``, ``strftime``, ``round``/``floor``/``ceil``, and conversion to :class:`PeriodIndex` (:issue:`66550`)
...
- Bug in :meth:`Series.median` and :meth:`DataFrame.median` on object-dtype data holding ``numpy.datetime64`` or ``numpy.timedelta64`` objects returning a meaningless float instead of raising ``TypeError`` (:issue:`34671`)
...
- Bug in :meth:`Series.skew`, :meth:`Series.kurt`, :meth:`DataFrame.skew`, and :meth:`DataFrame.kurt` on object-dtype data silently converting strings to numbers, e.g. ``Series(["1", "2", "3"], dtype=object).skew()`` returned ``0.0`` instead of raising ``TypeError`` as the other statistics do (:issue:`34671`, :issue:`44008`)
- Bug in :meth:`DataFrame.median` with ``skipna=False`` discarding the imaginary part of complex columns whose median was well-defined, depending on where the first column holding a missing value sat; :meth:`Series.median` returned a real ``NaN`` for complex data in the same case (:issue:`68487`)
- Bug in :meth:`Series.median` and :meth:`DataFrame.median` discarding the imaginary part of complex data on installations without ``bottleneck`` (:issue:`34671`)
- Bug in :meth:`Series.median`, :meth:`Series.sem`, and :meth:`DataFrame.median` returning ``float64`` for ``float32`` data when the reduction produced a missing value, with ``skipna=False`` and a missing value present or with ``ddof`` at least the number of observations; :meth:`DataFrame.median` widened every column, not only the one holding the missing value (:issue:`68487`)
- Bug in :meth:`Series.var`, :meth:`Series.std`, :meth:`Series.sem`, and their :class:`DataFrame` counterparts raising ``TypeError`` for object-dtype data holding numbers that the other statistics accept, such as :class:`decimal.Decimal` or complex numbers (:issue:`34671`)
...
- Fixed bug in arithmetic between :class:`DataFrame` and an object-dtype :class:`arrays.NumpyExtensionArray` returning an unexpected dtype because ``NumpyEADtype`` was not recognized as object dtype (:issue:`64969`)
...
- Bug in :class:`Series` construction and :class:`DataFrame` column assignment from a ``numpy.ma.MaskedArray`` where ``.values`` returned a ``MaskedArray`` instead of a ``ndarray`` (:issue:`65458`)
...
- Bug in :meth:`DataFrame.convert_dtypes` and :meth:`Series.convert_dtypes` converting a float column to ``Int64`` one off from its true value when it held a value equal to ``2**63``, and raising a spurious ``RuntimeWarning`` for other floats outside the ``int64`` range (:issue:`68315`)
...
- Bug in reductions and other operations returning a NumPy scalar raising ``AttributeError: module 'numpy' has no attribute 'complex256'`` with the ``future.python_scalars`` option enabled on platforms without extended-precision floating point types, e.g. macOS arm64 and Windows (:issue:`68068`)
...
- Fixed bug in :func:`to_numeric` raising ``TypeError`` for a :class:`Series` with :class:`PeriodDtype`; it now returns the period ordinals, matching :class:`PeriodIndex` (:issue:`69448`)
...
- Fixed bug in :func:`to_numeric`, and the readers built on it such as :func:`read_html`, :func:`read_xml`, :func:`read_excel`, :func:`read_fwf` and :func:`read_csv` with ``engine="python"`` or ``dtype="category"``, where a value written with a leading ``+`` and leading zeros after the decimal point lost significant digits, so e.g. ``"+0.0001234567890123456789"`` kept only 14 of them and ``"+0.0000000000000000000000005"`` became ``0.0`` (:issue:`68311`)
...
- Bug in :meth:`Series.str.casefold`, :meth:`Series.str.encode`, :meth:`Series.str.findall`, :meth:`Series.str.index`, :meth:`Series.str.partition`, :meth:`Series.str.rfind`, :meth:`Series.str.rindex`, :meth:`Series.str.rpartition`, :meth:`Series.str.translate`, :meth:`Series.str.wrap` and their :class:`Index` counterparts with :class:`ArrowDtype` raising ``ArrowInvalid`` or returning a ``null`` dtype when the object was empty, the values were all missing, or the missing values were grouped into a chunk of their own (:issue:`66822`)
- Bug in :meth:`Series.str.casefold`, :meth:`Series.str.encode`, :meth:`Series.str.findall`, :meth:`Series.str.join`, :meth:`Series.str.partition`, :meth:`Series.str.rpartition`, :meth:`Series.str.translate`, :meth:`Series.str.wrap` and their :class:`Index` counterparts with :class:`ArrowDtype` downgrading a ``large_string`` input to a ``string``-based result dtype (:issue:`66822`)
...
- Bug in :meth:`Series.str.normalize`, :meth:`Series.str.zfill` and their :class:`Index` counterparts with PyArrow-backed string dtypes raising ``ArrowInvalid`` or returning a ``null`` dtype when the object was empty, the values were all missing, or the missing values were grouped into a chunk of their own (:issue:`66822`)
- Bug in addition and multiplication on length-zero ``string``-dtype :class:`Series` and :class:`Index` returning ``object`` dtype instead of the string dtype, and raising ``TypeError`` when the other operand was length-zero with a different string storage or ``object`` dtype (:issue:`40624`)
...
- Bug in :meth:`Series.loc` and :meth:`DataFrame.loc` setitem-with-expansion raising a spurious ``RuntimeWarning`` for a ``float16`` value, even though every ``float16`` value fits every integer dtype it could expand into (:issue:`68315`)
...
- Bug in :meth:`Series.reindex`, :meth:`DataFrame.reindex`, :meth:`Index.get_indexer`, :meth:`Index.join` and :meth:`Index.difference` raising ``ValueError: Mixed timezones detected`` when an object-dtype :class:`Index` mixing :class:`datetime.date` objects with timezone-aware :class:`Timestamp` values was looked up in a :class:`DatetimeIndex` (:issue:`68577`)
- Bug in partial string indexing on a :class:`DatetimeIndex` whose ``freq`` is a generic :class:`DateOffset` (e.g. ``DateOffset(days=1, hours=-2)``, or any such index reversed) raising ``KeyError`` for a quarterly string such as ``"2022Q1"`` (:issue:`48210`)
...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem raising ``ValueError`` when assigning a 1-D list-like to several rows of a column picked out by a key matching exactly one column, such as a length-1 list, a length-1 slice, or a boolean mask (e.g. ``df.iloc[[0, 1], [2]] = [3, 4]``), on a single-dtype :class:`DataFrame`; mixed-dtype frames already accepted it (:issue:`68021`)
- Bug in :meth:`DataFrame.iloc` and :meth:`Series.iloc` setitem raising ``AttributeError: 'Index' object has no attribute 'sum'`` when the row key was a boolean :class:`Index` (:issue:`68021`)
...
- Bug in boolean-mask setitem, :meth:`Series.where`, :meth:`Series.mask`, and :meth:`Series.combine_first` aborting the Python process when applied to an :class:`ArrowDtype` column with null type (:issue:`66703`)
...
- Bug in setitem (e.g. :meth:`Series.iloc`), :meth:`Series.fillna`, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.replace`, :meth:`Series.reindex` and their :class:`DataFrame` counterparts on an :class:`ArrowDtype` ``timestamp`` column silently shifting a scalar :class:`Timestamp` or ``datetime.datetime`` value by the UTC offset when the value and the column disagreed about being timezone-aware; these now raise ``TypeError`` (:issue:`69029`)
...
- Bug in setitem, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.fillna` and their :class:`DataFrame` counterparts on :class:`ArrowDtype` data silently reinterpreting a value through its integer storage representation, or raising ``AttributeError``, instead of raising ``TypeError`` when the value's dtype and the column's disagree (:issue:`68419`)
- Bug in setitem, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.fillna` and their :class:`DataFrame` counterparts on :class:`ArrowDtype` temporal data raising ``ArrowNotImplementedError`` instead of assigning NA when the value was an all-NA numeric or boolean array, such as the column produced by aligning on a non-overlapping index (:issue:`68419`)
...
- Bug in :meth:`MultiIndex.union` and :meth:`MultiIndex.append` raising ``ValueError: Mixed timezones detected`` when a level of one :class:`MultiIndex` held :class:`datetime.date` objects and the matching level of the other held timezone-aware :class:`Timestamp` values (:issue:`68577`)
...
I/O
^^^
- :func:`read_csv` and :func:`read_table` now raise an informative ``ValueError`` for ``sep="\r"`` or ``delimiter="\r"``, which were silently mis-parsed into a frame of one column plus all-NaN padding, matching the error already raised for ``"\n"``; the error message no longer misattributes the failure to the ``python`` engine, and passing a different ``lineterminator`` is still allowed (:issue:`43528`, :issue:`51801`)
- :func:`read_csv` and :func:`read_table` now apply ``na_values`` and the default NA strings to the values a ``converters`` entry returns; previously the default ``c`` engine silently ignored both for a converter column, so an empty field under ``converters={"a": str}`` now reads as ``NaN`` (:issue:`13302`)
...
- :func:`read_csv` with ``engine="python"`` and ``index_col=False`` now honors ``on_bad_lines``, matching the ``c`` engine; previously a row with more fields than the first data row was truncated whatever ``on_bad_lines`` asked for, so ``"error"`` did not raise and a callable was never called (:issue:`49279`)
...
- :func:`read_csv`, :func:`read_fwf`, and :func:`read_table` now raise ``ValueError`` for a non-bool ``cache_dates``, ``dayfirst``, ``doublequote``, ``iterator``, ``keep_default_na``, ``low_memory``, ``memory_map``, ``na_filter``, ``skip_blank_lines`` or ``skipinitialspace``; previously the value was taken for its truthiness, so e.g. ``keep_default_na="False"`` silently did the opposite of what was asked. :func:`read_excel` (for ``keep_default_na`` and ``na_filter``) and :func:`read_html` (for ``keep_default_na``) now raise for the same reason (:issue:`68341`)
- :func:`read_html` and :func:`read_json` now raise the underlying error when a file cannot be read -- ``IsADirectoryError``, ``PermissionError``, an over-long-path ``OSError``, or for :func:`read_json` a ``URLError`` from a failed fetch -- instead of raising ``FileNotFoundError`` for every such failure (:issue:`29125`)
...
- :func:`read_pickle` now preserves the ``name`` of a :class:`Series` pickled by pandas before 2.1; previously :meth:`Series.copy`, :meth:`Series.reindex`, :meth:`Series.astype` and other methods silently dropped it, and re-pickling the object carried the problem into the new file (:issue:`61819`)
- :func:`read_sas` now raises ``OutOfBoundsDatetime`` for a SAS7BDAT date or datetime too far out of range to represent, instead of silently reading it as ``NaT``, and the error names the offending value rather than a date the file does not hold (:issue:`47339`)
- :func:`read_sas` now raises an informative ``ValueError`` for a SAS7BDAT file that changes the column layout it declared partway through, instead of parsing the rest of the file with metadata that no longer describes it -- reading past the end of a row and returning unrelated memory as data, reading every column from the wrong bytes, silently emptying a column to all-NaN, or padding the result with rows the file does not hold (:issue:`47339`)
...
- :func:`read_sas` now raises an informative ``ValueError`` for a SAS7BDAT file whose header claims a mix page holds more rows than fit on a page, instead of reading past the rows into the page padding and the metadata stored behind them and returning those bytes as data (:issue:`68067`)
...
- :func:`read_stata` now raises ``OutOfBoundsDatetime`` for a date too far from the epoch to represent in any of the Stata date formats, instead of silently decoding it to an unrelated date -- a far-future value could come back as a far-past one, or as ``NaT`` -- and no longer leaks a bare numpy ``RuntimeWarning``; previously only far-future ``%td`` values raised (:issue:`36096`)
- :meth:`DataFrame.to_clipboard` and :meth:`Series.to_clipboard` with ``excel=True`` now raise the underlying ``TypeError`` for an invalid keyword argument or value, such as ``index_names=False``; previously every ``TypeError`` was misreported as a warning about the ``sep`` argument and excel mode was abandoned for the ``to_string`` path (:issue:`44120`)
...
- Bug in :func:`read_csv` and :func:`read_table` with the default ``c`` engine raising ``ValueError`` for ``sep="\n"`` when a different ``lineterminator`` was given, instead of parsing the newline as an ordinary separator (:issue:`51801`)
- Fixed bug in :func:`read_csv` and :func:`read_table` where an explicit ``delimiter=None`` was treated as if no separator had been passed, so the default separator was used instead of detecting the separator as ``sep=None`` does (:issue:`47024`)
...
- Fixed bug in :func:`read_csv` where a non-ASCII single-character ``sep`` was handed to the ``c`` engine instead of falling back to the ``python`` engine on platforms whose filesystem encoding is not UTF-8, silently mis-splitting the file or raising ``UnicodeDecodeError`` (:issue:`46456`)
...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` where ``skiprows`` was silently ignored unless ``header=None``, so the rows meant to be skipped were read as the header and as data; a negative ``skiprows`` now skips nothing as the other engines do, instead of raising ``ParserError`` (:issue:`48507`)
...
- Fixed bug in :func:`read_csv`, :func:`read_fwf`, and :func:`read_table` with a pyarrow-backed integer ``dtype`` (e.g. ``dtype="int64[pyarrow]"``) where a missing value in the column sent the rest of it through ``float64``, so integers above ``2**53`` came back rounded (:issue:`56135`)
...
- Fixed bug in :func:`read_csv` with the ``c`` engine and a ``thousands`` separator where numbers written with leading zeros (e.g. ``"000000000010084566"`` or ``"0.00564728024629623"``) lost low-order significant digits (:issue:`64184`)
- Fixed bug in :func:`read_csv` with the ``c`` engine where ``encoding`` was ignored for a binary buffer that reports no ``mode``, such as an ``mmap.mmap`` object or a stream from ``boto3``/``botocore``, so non-UTF-8 data raised ``UnicodeDecodeError`` or was silently decoded to the wrong characters (:issue:`52252`)
...
- Fixed bug in :func:`read_csv` with the ``c`` engine where a run of rows with fewer fields than expected corrupted the parse of the rows after it, raising a spurious ``ParserError`` reporting ``Buffer overflow caught - possible malformed input file`` on ordinary ragged input, and truncating a much wider field later in the same read or crashing the interpreter (:issue:`66657`)
...
- Fixed bug in :func:`read_csv` with the ``c`` engine where an integer column holding ``-9223372036854775808`` (the smallest ``int64``) or ``18446744073709551615`` (the largest ``uint64``) read that value as missing when ``dtype_backend`` was ``"numpy_nullable"`` or ``"pyarrow"``; with the default backend, ``-9223372036854775808`` was also read as missing in a column that held a genuine missing value too (:issue:`66855`)
...
- Fixed bug in :func:`read_csv` with the ``python`` engine, :func:`read_fwf` and :func:`read_excel` where ``skipfooter`` counted the rows it had parsed rather than the lines of the file, so lines the parser had already skipped -- ``skiprows`` lines, and blank or comment lines above the data -- did not count towards it and too many rows were dropped from the end, sometimes leaving an empty ``DataFrame`` (:issue:`36827`)
...
- Fixed bug in :func:`read_excel` and :meth:`DataFrame.to_excel` with ``engine="odf"`` where line breaks inside a cell were lost when writing, and dropped when reading files that store them as ``<text:line-break/>`` elements or multiple ``<text:p>`` paragraphs (:issue:`55728`, :issue:`53924`)
...
- Fixed bug in :func:`read_feather`, :func:`read_parquet`, :func:`read_orc` and the other readers that build a frame from an Arrow table where, with the ``future.infer_string`` option disabled, an *ordered* categorical column came back with ``str`` categories instead of the ``object`` categories an unordered one gets (:issue:`64659`)
...
- Fixed bug in :func:`read_sas` where a SAS7BDAT date column holding a fractional day count was read at midnight, dropping the time of day, and for a date before 1960 at the *following* midnight, giving the wrong date (:issue:`47339`)
- Fixed bug in :func:`read_sas` where a SAS7BDAT reader created with ``iterator=True`` or ``chunksize`` held the file open after the caller dropped it, until the next cyclic garbage collection, instead of closing it right away (:issue:`68973`)
...
- Fixed buffer overflow in :meth:`DataFrame.to_json` and :meth:`Series.to_json` that corrupted the output or crashed the interpreter when serializing deeply nested values with ``indent``, or values holding strings that need escaping (:issue:`67929`)
...
- Fixed memory leak in :func:`read_csv` with ``engine="c"`` when a string or ``category`` column failed to decode, for example invalid UTF-8 under the default ``encoding_errors="strict"`` (:issue:`67931`)
...
- Bug in :meth:`DataFrame.to_json` and :meth:`Series.to_json` silently shifting the serialized instant by up to 59 seconds for a timezone whose UTC offset is not a whole number of minutes, such as any zone in its pre-standardization local mean time era (``Asia/Tokyo`` is ``+09:18:59`` before 1888) (:issue:`66818`)
...
- :meth:`HDFStore.append` now raises ``TypeError`` when the appended data is timezone-aware and the stored column is timezone-naive, or vice versa; such an append was previously accepted and the rows already on disk were read back as different timestamps (:issue:`68583`)
- :meth:`HDFStore.append` now raises ``TypeError`` when the appended index and the stored one disagree about being a :class:`PeriodIndex`, or are both ``PeriodIndex`` with a different ``freq``; such an append was previously accepted and the rows already on disk were read back as the wrong values (:issue:`68523`)
- :meth:`HDFStore.append` now raises ``ValueError`` when appending a string equal to the sentinel an existing string column already uses for its missing values, instead of silently reading that value back as missing; because the sentinel can also be longer than the values themselves, appending a missing value to a narrow existing column may now require the table to have been created with a larger ``min_itemsize`` (:issue:`9604`)
...
- :meth:`HDFStore.remove` now raises ``TypeError`` for a ``None`` operand in a ``where`` query (e.g. ``where="A == None"``); against a boolean data column such a query previously deleted rows instead of raising (:issue:`68642`)
...
- :meth:`HDFStore.select`, :meth:`HDFStore.remove` and :func:`read_hdf` now warn when a nested ``where`` of the form ``"(A & B) | (C & D)"`` over indexed columns may match the wrong rows because of an upstream PyTables bug, suggesting writing with ``index=False`` or splitting the ``OR`` branches into separate operations (:issue:`50598`)
- :meth:`HDFStore.select`, :meth:`HDFStore.select_as_coordinates`, :meth:`HDFStore.select_as_multiple` and :meth:`HDFStore.remove` now raise an informative ``ValueError`` naming the offending row locations when a ``where`` given as row coordinates falls outside the table or outside the requested ``start``/``stop`` window (e.g. ``store.select("df", where=[100])`` on a 10-row table), instead of a misleading ``TypeError`` reporting that ``where`` must be a string or ``PyTablesExpr``. :meth:`HDFStore.select_as_coordinates`, :meth:`HDFStore.remove`, and reads with ``iterator=True`` or ``chunksize`` previously skipped the range check entirely, so a negative coordinate wrapped around and returned the wrong row (e.g. ``where=[-3]`` returning row 7) rather than raising (:issue:`68029`)
...
- Bug in :meth:`HDFStore.select` where a ``where`` query comparing an integer column against a value that is not a whole number (e.g. ``"col == 1.5"``) silently compared against the nearest whole number instead, so ``"col == 1.5"`` matched rows holding ``2``, ``"col > 1.5"`` skipped them, and ``"col != 1.5"`` dropped them (:issue:`68032`)
...
- Fixed bug in :func:`read_hdf` where a categorical column written with ``format="table"`` whose categories included the string used on disk for missing values (e.g. the default ``"nan"``) raised ``ValueError: operands could not be broadcast together`` on read, and where appending such a categorical to an existing key raised ``ValueError: cannot append a categorical with different categories to the existing``; that category now round-trips (:issue:`21741`)
...
- Fixed bug in :func:`read_hdf` where a string :class:`Index` or :class:`MultiIndex` level was read back with ``object`` dtype instead of ``str`` whenever the rows being read held no non-missing string, so the index dtype depended on the selection: an empty ``where`` result, a ``start``/``stop`` slice or a ``chunksize`` chunk covering only missing values, and an all-missing index were all affected (:issue:`9604`)
- Fixed bug in :func:`read_hdf` where a string column written with ``format="table"`` did not round-trip a literal ``"nan"``, which was read back as a missing value even when the column held no missing values at all; a column's missing values are now stored with a sentinel that collides with none of its values, unless an explicit ``nan_rep`` is passed (:issue:`9604`)
- Fixed bug in :func:`read_hdf` where reading a fixed-format (default) object with a :class:`MultiIndex` and ``start``/``stop`` returned an index built from the wrong labels, or raised ``ValueError: On level 0, code max ... >= length of level`` (:issue:`68038`)
- Fixed bug in :func:`read_parquet` crashing the interpreter when called with ``to_pandas_kwargs={"self_destruct": True}`` (:issue:`66509`)
- Fixed bug in :meth:`DataFrame.to_clipboard` and :func:`read_clipboard` where the Qt clipboard backend (used on Linux when ``xclip``, ``xsel`` and ``wl-clipboard`` are all unavailable) permanently switched the process to the environment's locale, so afterwards e.g. ``to_datetime("March 1, 2018", format="%B %d, %Y")`` raised ``ValueError`` on a machine whose locale is not English (:issue:`44625`)
- Fixed bug in :meth:`DataFrame.to_excel` which no longer includes the cell content length in the warning message emitted when cell contents exceed Excel's 32767-character limit, so duplicate warnings are no longer emitted (:issue:`67470`)
...
- Fixed bug in :meth:`HDFStore.select` where a ``<`` or ``<=`` ``where`` query on a ``datetime64`` or ``timedelta64`` column also matched the rows holding ``NaT`` (:issue:`68031`)
...
- Fixed bug in :meth:`HDFStore.select` where a ``where`` query using ``!=`` against a list of two or more values (e.g. ``"col != [2, 3]"``) silently returned every row instead of excluding the listed ones; a single-value list was unaffected (:issue:`68030`)
...
- Fixed bug in :meth:`HDFStore.select_as_coordinates` where a negative ``start`` or ``stop``, or a ``stop`` past the end of the table, returned coordinates that name no row; passing them back to :meth:`HDFStore.select` silently returned rows from the start of the table instead (:issue:`68033`)
- Fixed bug in :meth:`HDFStore.select` where a negative ``start`` was not counted back from the end of the table when the read went through row coordinates: a boolean-mask ``where`` raised ``IndexError``, and reading with ``iterator=True`` or ``chunksize`` silently returned rows more than once (:issue:`68033`)
...
- Bug in :class:`Period` subtraction raising ``TypeError`` instead of returning an empty result when either operand was a length-zero :class:`PeriodIndex` or ``period``-dtype :class:`Series` (:issue:`40624`)
...
Plotting
^^^^^^^^
- Bug in :meth:`DataFrame.plot.bar` and :meth:`DataFrame.plot.barh` with ``stacked=True`` raising ``IndexError`` when a ``subplots`` group of a single column was listed before a group of several columns (:issue:`67726`)
...
- Bug in :meth:`.DataFrameGroupBy.sum` and :meth:`.SeriesGroupBy.sum` on ``timedelta64`` data where a group total that left the representable range silently wrapped to an unrelated value, or landed on the ``NaT`` sentinel and came back as a missing value, instead of raising ``OutOfBoundsTimedelta`` (:issue:`66551`)
...
- Bug in :meth:`.GroupBy.indices` and :meth:`.GroupBy.get_group` returning the wrong group's rows for a multi-key groupby with a :class:`Categorical` key, ``sort=False``, and ``observed=False`` when the data's first-appearance order differed from the category order (:issue:`66893`)
...
- Bug in :meth:`.Rolling.skew` and :meth:`.Rolling.kurt` returning ``NaN`` for every later window once the values shared by two consecutive windows were all NA (:issue:`69037`)
...
- Bug in :meth:`DataFrame.groupby` and :meth:`Series.groupby` with ``observed=False`` on a :class:`Categorical` key alongside another key, where rows dropped by ``dropna=True`` for an NA key raised ``IndexError``, or with ``sort=False`` returned the groups in the wrong order or duplicated one of them (:issue:`68931`)
...
- Bug in ``rolling``, ``expanding`` and ``ewm`` operations on a :class:`ArrowDtype` timestamp or duration dtype silently returning ``float64`` counts instead of raising ``DataError`` like the equivalent NumPy ``datetime64`` or ``timedelta64`` dtype (:issue:`66445`)
...
- Bug in :func:`concat` and :meth:`Series.combine_first` silently dropping the timezone of an :class:`ArrowDtype` ``timestamp`` column, shifting its values by the UTC offset, when it was combined with a ``timestamp`` column of a different dtype; the result is now ``object`` when the two timezones disagree, and otherwise keeps the timezone, including against a timezone-aware ``datetime64`` column that previously gave ``object`` (:issue:`69029`)
...
- Bug in :func:`merge_asof` where a negative tolerance raised ``MergeError`` with the misleading message "tolerance must be positive" even though ``tolerance=0`` is accepted; the message now reads "tolerance must be non-negative" (:issue:`66289`)
...
- Bug in :func:`qcut` not validating ``q``, so a non-positive integer or a ``q`` that was neither an integer nor list-like failed with ``IndexError``, ``TypeError`` or no error at all instead of ``ValueError`` (:issue:`67406`)
...
- Bug in :meth:`DataFrame.describe` raising ``ValueError: The truth value of an array with more than one element is ambiguous`` when ``include`` was an :class:`Index`, :class:`Series`, or ndarray holding more than one dtype spec (:issue:`68470`)
- Bug in :meth:`DataFrame.melt` and :func:`wide_to_long` returning an ``object`` dtype result column instead of preserving :class:`IntervalDtype` when the frame had no rows or every melted value was ``NaN`` (:issue:`68925`)
...
- Bug in :meth:`DataFrame.select_dtypes` where a timezone-naive spec such as ``"datetime64[s]"`` or ``np.datetime64`` selected timezone-aware pyarrow-backed timestamp columns; these are now skipped, as timezone-aware numpy columns already were (:issue:`68075`)
- Bug in :meth:`DataFrame.select_dtypes` where a unit-qualified spec such as ``"datetime64[ms]"`` selected pyarrow-backed ``date32``/``date64`` columns, which hold dates rather than datetimes of that resolution; such columns are now selected only by the unitless ``"datetime"``/``"datetime64"``/``np.datetime64`` specs (:issue:`68488`)
- Bug in :meth:`DataFrame.select_dtypes` where explicitly naming the ``str`` dtype did not stop an ``object`` spec from also matching string columns, so ``include=["str", "number"]`` with ``exclude="object"`` dropped the string columns; excluding ``object`` alone now also emits the ``object``/``str`` deprecation warning that including it already did (:issue:`68454`)
...
- Bug in :meth:`DataFrame.unstack`, :meth:`Series.unstack`, :meth:`DataFrame.pivot`, :meth:`DataFrame.pivot_table`, and :func:`crosstab` where the ``PerformanceWarning`` about the size of the reshaped result was not raised, or reported too few cells, for a :class:`MultiIndex` with more than one level left after unstacking or with missing values in any level (:issue:`10582`)
...
- Bug in :class:`SparseArray` and :class:`SparseDtype` accepting ``datetime64`` and ``timedelta64`` units pandas cannot hold: an unsupported resolution in the data is now cast to a supported one and an unsupported ``dtype=`` now raises, both matching the dense constructors, where previously a multiplier unit such as ``M8[10s]`` built a column that could not be displayed (:issue:`68522`)
- Bug in :class:`SparseArray` with a datetime64 or timedelta64 subtype where a ``fill_value`` given as a :class:`Timestamp` or :class:`Timedelta` was silently truncated to microsecond resolution by :meth:`SparseArray.to_dense`, :meth:`SparseArray.value_counts`, :meth:`SparseArray.unique`, and arithmetic, and made ``np.asarray`` return ``object`` dtype holding raw integers, which the repr printed in place of the values; all of these behaved correctly for an equivalent ``np.datetime64`` fill value (:issue:`68589`)
- Bug in :class:`SparseArray` with a datetime64 or timedelta64 subtype where a ``np.nan`` or ``pd.NA`` ``fill_value`` made ``np.asarray`` return an ``object`` array whose gaps held the fill value instead of ``NaT``, and whose remaining entries came back as raw integers on a nanosecond unit; :meth:`SparseArray.unique` raised ``ValueError``, and for a ``pd.NA`` fill value :meth:`SparseArray.to_dense` raised ``ValueError`` and comparison operations raised ``TypeError`` (:issue:`68558`)
...
- Bug in :class:`SparseDtype` comparing equal to any dtype with the same subtype and a fill value of the same Python type once either side's fill value was NA, e.g. ``Sparse[float64, nan] == Sparse[float64, 1.5]``; among other things this made :meth:`SparseArray.astype` to such a dtype silently return the original array (:issue:`68582`)
...
- Bug in :func:`concat` and :meth:`Series.shift` on a :class:`SparseDtype` column with an ``object`` subtype holding only datetimelike values silently returning a ``datetime64`` or ``timedelta64`` subtype instead (:issue:`69028`)
- Bug in :func:`concat`, :meth:`Index.union`, :meth:`Index.append`, and :meth:`DataFrame.combine_first` combining two :class:`SparseDtype` objects resolving the subtype with numpy's promotion rules instead of pandas': a boolean subtype with a numeric one raised ``ValueError: fill_value must be a valid value for the SparseDtype.subtype``, and :meth:`DataFrame.combine_first` on two datetime64 or timedelta64 subtypes gave ``Sparse[object]`` instead of keeping the subtype (:issue:`69028`)
- Bug in :func:`concat`, :meth:`Series.reindex`, :meth:`DataFrame.reindex`, :meth:`GroupBy.first`, :meth:`GroupBy.last`, and :meth:`SparseArray.take` returning the wrong subtype for :class:`SparseDtype` data: a narrow float subtype widened to ``float64``, ``uint64`` values above ``2**53`` were silently rounded, and a datetime64 or timedelta64 subtype changed unit, became ``object`` holding raw integers, or raised ``DTypePromotionError`` (:issue:`26123`, :issue:`26288`, :issue:`68469`)
- Bug in :meth:`DataFrame.astype`, :meth:`Series.astype`, and :meth:`SparseArray.astype` silently changing the data when given a :class:`SparseDtype` with a different ``fill_value``: every position holding the old ``fill_value`` came back as the new one, which also made :func:`concat` and ``axis=1`` reductions wrong on sparse data with differing fill values. Casting missing gaps to an integer subtype now raises ``IntCastingNaNError`` as it does for dense data (:issue:`35795`)
...
- Bug in :meth:`DataFrame.idxmin` and :meth:`DataFrame.idxmax` on :class:`SparseDtype` columns returning a wrong label with ``skipna=False`` instead of raising when the column held a missing value, and emitting a spurious ``ComplexWarning`` on complex-valued columns (:issue:`68387`)
- Bug in :meth:`DataFrame.median` and :meth:`DataFrame.sem` on :class:`SparseDtype` columns reporting a different subtype than the equivalent dense column when the result is a missing value (:issue:`68487`)
- Bug in :meth:`DataFrame.prod`, :meth:`DataFrame.median`, :meth:`DataFrame.var`, :meth:`DataFrame.std`, :meth:`DataFrame.sem`, :meth:`DataFrame.skew`, :meth:`DataFrame.kurt`, :meth:`DataFrame.describe`, and their :class:`Series` counterparts raising ``TypeError: cannot perform <op> with type Sparse[...]`` on :class:`SparseDtype` data; :meth:`SparseArray.prod`, :meth:`SparseArray.median`, :meth:`SparseArray.var`, :meth:`SparseArray.std`, :meth:`SparseArray.sem`, :meth:`SparseArray.skew`, and :meth:`SparseArray.kurt` now exist to match (:issue:`68194`)
- Bug in :meth:`DataFrame.sparse.from_spmatrix` giving the matrix's unstored entries a missing value for a float or complex dtype, where SciPy defines them as zero, so the resulting frame did not hold the same values as ``spmatrix.toarray()`` (:issue:`59212`)
- Bug in :meth:`DataFrame.sparse.to_coo` writing zeros in the unstored positions of a column whose ``fill_value`` is not zero; it now raises ``ValueError`` (:issue:`69115`)
- Bug in :meth:`Index.insert` and :meth:`SparseArray.insert` silently casting a non-missing value the subtype cannot hold: inserting ``1.5`` into a ``Sparse[int64]`` :class:`Index` truncated it to ``1`` instead of widening to ``Sparse[float64]``, and a :class:`Timestamp` was stored as an integer (:issue:`69028`)
- Bug in :meth:`Series.any`, :meth:`Series.all`, :meth:`DataFrame.any`, :meth:`DataFrame.all`, :meth:`SparseArray.any`, and :meth:`SparseArray.all` mishandling missing values on :class:`SparseDtype` data: with ``skipna=True`` a missing value was reduced for its own truthiness instead of being skipped, and with ``skipna=False`` missing values were dropped instead of reduced; :meth:`SparseArray.any` and :meth:`SparseArray.all` also raised a ``TypeError`` when called with the ``skipna`` argument (:issue:`68390`)
- Bug in :meth:`Series.any`, :meth:`Series.all`, :meth:`DataFrame.any`, :meth:`DataFrame.all`, :meth:`SparseArray.any`, and :meth:`SparseArray.all` raising ``TypeError: boolean value of NA is ambiguous`` with ``skipna=False`` on numeric or ``timedelta64`` :class:`SparseDtype` data whose fill value is ``pd.NA``, which is what sparsifying a nullable numeric array produces (:issue:`68559`)
- Bug in :meth:`Series.any`, :meth:`Series.all`, :meth:`DataFrame.any`, :meth:`DataFrame.all`, :meth:`SparseArray.any`, and :meth:`SparseArray.all` returning a result on datetime64 :class:`SparseDtype` data instead of raising ``TypeError`` the way the dense equivalents do (:issue:`68438`)
- Bug in :meth:`Series.argmin`, :meth:`Series.argmax`, :meth:`Series.idxmin`, :meth:`Series.idxmax`, :meth:`DataFrame.idxmin`, :meth:`DataFrame.idxmax`, :meth:`SparseArray.argmin`, and :meth:`SparseArray.argmax` raising ``ValueError: attempt to get argmin of an empty sequence`` on :class:`SparseDtype` data with no non-missing stored values, when the non-missing ``fill_value`` occupies at least one position, instead of returning the first such position (:issue:`68462`)
- Bug in :meth:`Series.astype`, :meth:`DataFrame.astype`, :meth:`Index.astype`, and :meth:`SparseArray.astype` from a :class:`SparseDtype` to a dense dtype silently rounding ``uint64`` values above ``2**53``, and, for a datetime64 or timedelta64 subtype with a boxed :class:`Timestamp` or :class:`Timedelta` ``fill_value``, silently giving wrong values or raising ``TypeError`` (:issue:`68573`)
- Bug in :meth:`Series.cumsum`, :meth:`DataFrame.cumsum`, and :meth:`SparseArray.cumsum` on :class:`SparseDtype` data propagating a missing value into every later entry instead of skipping it, when that value is held among the stored values rather than as a gap (:issue:`68972`)
...
- Bug in :meth:`Series.fillna`, :meth:`DataFrame.fillna`, and :meth:`SparseArray.fillna` on a datetime64 or timedelta64 :class:`SparseArray` storing ``object``-dtype or wrong-unit values under a dtype still reporting the original subtype, or raising ``DTypePromotionError``, depending on how the fill value was spelled (:issue:`69027`)
- Bug in :meth:`Series.mean` with ``skipna=False`` ignoring missing values for :class:`SparseDtype`-backed :class:`Series` (:issue:`65478`)
- Bug in :meth:`Series.mean`, :meth:`DataFrame.mean`, and :meth:`SparseArray.mean` on :class:`SparseDtype` data with no non-missing values, an empty array included, emitting a numpy ``RuntimeWarning``, or raising ``ZeroDivisionError`` for an object subtype or ``TypeError`` for a non-numeric ``fill_value``; the missing value is now returned quietly (:issue:`68484`)
- Bug in :meth:`Series.memory_usage`, :meth:`DataFrame.memory_usage`, and :meth:`Index.memory_usage` with ``deep=True``, :meth:`DataFrame.info` with ``memory_usage="deep"``, and ``sys.getsizeof`` raising ``TypeError`` on an object subtype :class:`SparseDtype`; :meth:`SparseArray.memory_usage` is new (:issue:`68490`)
- Bug in :meth:`Series.min`, :meth:`Series.max`, :meth:`Series.median`, :meth:`Series.std`, and the matching :class:`SparseArray` methods returning a ``numpy.datetime64`` or ``numpy.timedelta64`` on datetime64 or timedelta64 :class:`SparseDtype` data instead of a :class:`Timestamp` or :class:`Timedelta`, and returning ``numpy.datetime64("NaT")`` rather than ``NaT`` for a missing result; :meth:`Series.mean` and :meth:`Series.sum` were affected for timedelta64 only (:issue:`68555`)
...
- Bug in :meth:`Series.sum`, :meth:`DataFrame.sum`, and :meth:`SparseArray.sum` on :class:`SparseDtype` data with an object subtype and a non-missing ``fill_value``, concatenating the fill values after the stored ones instead of summing in the original order, and raising ``TypeError`` where the dense equivalent returns a result (:issue:`68581`)
...
- Bug in :meth:`Series.to_numpy`, :meth:`DataFrame.to_numpy`, and casting to ``object`` on a datetime64 or timedelta64 :class:`SparseArray` whose ``fill_value`` is a :class:`Timestamp`, :class:`Timedelta`, or ``np.nan``, returning ``object`` dtype mixing ``datetime.datetime`` values or raw integers with the fill value (:issue:`68590`)
...
- Bug in :meth:`SparseArray.map` and :meth:`Series.map` on :class:`SparseDtype` data raising ``TypeError: boolean value of NA is ambiguous`` for an ``object`` subtype holding ``pd.NA`` under a non-missing fill value (:issue:`68586`)
...
- Bug in :meth:`SparseArray.to_dense` and :meth:`Series.to_numpy` with an explicit ``dtype`` on a datetime64 or timedelta64 :class:`SparseArray` mishandling the ``fill_value``: a nanosecond :class:`Timestamp` or :class:`Timedelta` was truncated to microseconds, and a ``fill_value`` whose unit differed from the column's was read as a raw integer (:issue:`68590`)
- Bug in :meth:`SparseArray.value_counts` with ``dropna=False`` raising ``TypeError: boolean value of NA is ambiguous`` for an ``object`` subtype holding ``pd.NA`` under a non-missing fill value (:issue:`68586`)
- Bug in NumPy ufunc accumulations such as ``np.add.accumulate`` and ``np.maximum.accumulate`` on a :class:`SparseArray` raising ``TypeError: cannot accumulate on a scalar`` (:issue:`68566`)
...
- Bug in arithmetic and comparison ops between :class:`SparseDtype` data and a datetime64 or timedelta64 operand raising ``ValueError``; invalid ops now raise ``TypeError`` as the dense equivalents do, and valid ops such as multiplying integers by timedeltas now return a result instead of raising (:issue:`68466`)
- Bug in arithmetic and logical ops between a :class:`SparseArray` and an operand it has no common dtype with, such as an integer :class:`SparseArray` and a timedelta64 array, returning ``Sparse[object]`` instead of the dtype the dense equivalent returns; the logical ops also returned integers where the dense equivalent returns booleans (:issue:`68562`)
- Bug in binary operations on :class:`SparseDtype` data storing values equal to the result's ``fill_value``, so indexing with a comparison result selected the wrong elements and the result reported more stored values than it holds (:issue:`45126`, :issue:`45284`)
- Bug in comparison and logical operators between a :class:`SparseDtype`-backed :class:`Series` and the ``pd.NA`` scalar raising ``TypeError: boolean value of NA is ambiguous`` instead of matching the non-sparse equivalent (:issue:`68579`)
- Bug in comparison operators between a :class:`SparseDtype`-backed :class:`Series` and a nullable ``boolean`` :class:`Series` raising ``TypeError: boolean value of NA is ambiguous``; the result now matches the non-sparse equivalent and has ``boolean`` dtype (:issue:`68586`)
- Bug in comparison operators between a :class:`SparseDtype`-backed :class:`Series` and a nullable integer or floating :class:`Series` silently resolving the missing values as if they were ``NaN``; the result now matches the non-sparse equivalent and has ``boolean`` dtype (:issue:`68579`)
- Bug in comparison operators between two ``object`` subtype :class:`SparseArray` objects with different sparse indices raising ``AttributeError: module 'pandas._libs.sparse' has no attribute 'sparse_eq_object'`` (:issue:`68586`)
- Bug in comparison operators with an ``object`` subtype :class:`SparseArray` on either side raising ``TypeError`` for a stored ``pd.NA`` or for a missing scalar operand, and reporting a missing scalar as equal to a stored missing value; the result now matches the dense equivalent (:issue:`68586`)
- Bug in indexing a :class:`SparseArray` with a boolean :class:`SparseArray` mask of the wrong length not raising ``IndexError`` (:issue:`45284`)
- Bug in indexing a :class:`SparseArray` with a boolean :class:`SparseArray` mask returning the wrong elements, e.g. ``arr[arr > other]`` (:issue:`45284`)
...
- Bug in logical operators (``&``, ``|``, ``^``) between a :class:`SparseDtype`-backed :class:`Series` and a :class:`Categorical` operand answering from the truthiness of the operand's values instead of raising ``TypeError`` as the non-sparse equivalent does (:issue:`68569`)
...
- Bug in arithmetic, comparison, and logical operations between PyArrow-backed or :class:`StringDtype` data and a two-dimensional operand raising an assortment of internal or misleading errors, or silently returning a wrong result; these now raise ``NotImplementedError`` (:issue:`62682`)
- Bug in comparisons between PyArrow-backed data and a differently-sized array or list raising ``pyarrow.ArrowInvalid`` instead of reporting the length mismatch (:issue:`62682`)
...
- Bug in the :class:`Series` constructor with an index but no data raising ``RecursionError`` for an :class:`~pandas.api.extensions.ExtensionDtype` whose ``na_value`` is dict-like, e.g. a nested or arrow-backed dtype (:issue:`33900`, :issue:`41377`)
- Bug in :attr:`Series.list` raising error when using negative indices with PyArrow list dtypes (:issue:`63221`)
- Bug in the ``prod``, ``mean``, ``median``, ``std``, ``var``, ``sem``, ``skew`` and ``kurt`` methods of a :class:`.StringArray` (and ``np.prod``, ``np.mean``, ``np.std`` and ``np.var``, which dispatch to them) not rejecting the string dtype, returning a value or raising an unrelated error instead of the ``TypeError`` the equivalent :class:`Series` reduction raises (:issue:`68389`)
- Bug in the repr of a :class:`.NumpyExtensionArray` backed by a numpy bytes dtype rendering ``b"foo"`` as ``'b'foo''`` instead of ``b'foo'`` (:issue:`68080`)
...
Styler
^^^^^^
- Bug in :meth:`.Styler.to_typst` leaving empty cells for hidden index levels and columns (:issue:`64663`)
...
- Bug in :func:`testing.assert_series_equal` and :func:`testing.assert_frame_equal` raising ``TypeError`` instead of ``AssertionError`` when comparing object-dtype data containing nested lists of different lengths (:issue:`69014`)
...
- Bug in :meth:`DataFrame.__setitem__` and :meth:`Series.__setitem__` emitting a spurious :class:`errors.ChainedAssignmentError` warning for a valid assignment made from compiled code, e.g. from a Cython extension module (:issue:`51315`)
...
- Bug in :meth:`DataFrame.query` and :meth:`DataFrame.eval` raising ``UndefinedVariableError`` when accessing an attribute of a class with the ``@`` prefix, e.g. ``df.query("x == @A.a")`` (:issue:`48694`)
...
- Bug in :meth:`Series.shift` ignoring the ``suffix`` argument when ``periods`` was list-like, so the shifted columns were named ``a_1`` rather than ``a_suffix_1`` (:issue:`54806`)
- Bug in :meth:`Series.shift` with ``periods`` given as an ndarray or :class:`Index` of shifts, which returned an unshifted copy of the original Series for the single shift ``0`` and raised ``ValueError`` for more than one shift; a :class:`Series` of shifts raised for any number of shifts (:issue:`68942`)
...
- Bug in warning ``stacklevel`` resolution where warnings raised on behalf of user code living in a third-party package whose install directory merely starts with ``pandas`` (e.g. ``pandas_ta``) pointed past the real caller (:issue:`67992`)
