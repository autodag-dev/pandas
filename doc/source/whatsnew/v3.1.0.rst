...
- Fixed bug in :func:`read_excel` with ``dtype_backend="pyarrow"`` where ``index_col`` together with ``parse_dates=True`` returned a plain :class:`Index` with a ``timestamp`` :class:`ArrowDtype` instead of a :class:`DatetimeIndex`, so downstream operations such as ``index.to_period`` failed (:issue:`65524`)
