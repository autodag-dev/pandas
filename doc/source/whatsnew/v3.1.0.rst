...
- Deprecated setting an out-of-bounds numpy scalar into an array of a numpy-nullable integer dtype. The value is currently stored altered to fit (``np.int64(-1)`` becomes ``255`` in ``UInt8``); in a future version the behavior will change. Cast the array to a dtype that can hold the value first (:issue:`48867`)
