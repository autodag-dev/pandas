...
- Deprecated :meth:`Timestamp.timestamp` treating a tz-naive :class:`Timestamp` as UTC; in a future version it will treat it as local time, matching :meth:`datetime.datetime.timestamp`. Use ``ts.tz_localize("UTC").timestamp()`` to keep the current behavior (:issue:`50298`)
