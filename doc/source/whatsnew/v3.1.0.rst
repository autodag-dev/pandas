...
- Bug in :meth:`DataFrame.groupby` and :meth:`Series.groupby` on a :class:`Categorical` key with ``observed=False`` alongside another key, where rows dropped for a missing key value raised ``IndexError``, or with ``sort=False`` returned the groups in the wrong order or duplicated one of them (:issue:`68931`)
