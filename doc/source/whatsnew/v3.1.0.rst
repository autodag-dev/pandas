...
- :meth:`Series.dt.tz_convert`, :meth:`DatetimeIndex.tz_convert`, and :meth:`Timestamp.tz_convert` now accept a ``naive_tz`` keyword argument. If the data is tz-naive and ``naive_tz`` is provided, the timestamps are first localized to ``naive_tz`` and then converted to the target timezone, replacing the convert-or-assume pattern of ``x.tz_localize(tz) if x.tz is None else x.tz_convert(tz)`` (:issue:`66389`)
...
- Bug in :meth:`Series.str.match` and :meth:`Index.str.match` raising for regex flags other than ``re.IGNORECASE`` (e.g. ``re.MULTILINE``, ``re.DOTALL``, ``re.ASCII``), whether passed via ``flags`` or carried by a compiled ``re.Pattern`` (:issue:`63108`)
