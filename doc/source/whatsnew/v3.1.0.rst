...
- Bug in :meth:`DatetimeIndex.tz_convert`, :meth:`DatetimeIndex.tz_localize`, :meth:`Series.dt.tz_convert`, :meth:`Series.dt.tz_localize`, :meth:`Timestamp.tz_convert`, :meth:`Timestamp.tz_localize` and :func:`date_range` raising ``TypeError`` or ``ZoneInfoNotFoundError``, or silently using the installed ``tzdata`` rules instead of the ones supplied, with a ``zoneinfo`` timezone built by ``ZoneInfo.from_file`` (:issue:`66828`)
