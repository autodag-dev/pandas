...
- Clarified in the :meth:`DataFrame.to_json` and :func:`read_json` docstrings that the default ``date_format='epoch'`` round-trip is lossy unless a matching ``date_unit`` is passed to both sides (:issue:`66615`)
