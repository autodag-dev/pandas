...
- Bug in :meth:`DataFrame.where` and :meth:`DataFrame.mask` with a datetimelike dtype (e.g. ``datetime64[ns]``, ``period[D]``) silently filling masked positions from the wrong entries of a list ``other`` (:issue:`63842`)
- Bug in :meth:`DataFrame.where`, :meth:`DataFrame.mask`, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Index.where`, :meth:`Index.putmask` and boolean-mask setitem with an extension dtype (e.g. ``str``, ``Int64``, :class:`ArrowDtype`) casting to ``object`` or raising ``TypeError`` for a list ``other`` (:issue:`63842`)
