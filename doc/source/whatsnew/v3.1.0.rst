...
- Bug in setitem, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.fillna` and their :class:`DataFrame` counterparts on :class:`ArrowDtype` data silently reinterpreting a value through its integer storage representation instead of raising ``TypeError`` when the value's dtype and the column's disagree, e.g. assigning a :class:`DatetimeIndex` into an ``int64[pyarrow]`` column stored its epoch values, and assigning an integer array into a ``timestamp[ns][pyarrow]`` or ``duration[ns][pyarrow]`` column read it back as timestamps or timedeltas; a scalar ``Timestamp`` in the same position raised ``AttributeError`` (:issue:`68419`)
