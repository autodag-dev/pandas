...
- Deprecated ``__invert__`` (the ``~`` operator) on object dtype. ``~`` dispatches to each element's own ``__invert__``, so a boolean mask that has been cast to object silently becomes integers instead of being negated. Cast to a boolean or integer dtype first, e.g. ``~obj.astype(bool)`` (:issue:`16873`, :issue:`31035`, :issue:`51567`)
