...
- Bug in :meth:`ExtensionArray.factorize` where pre-encoded ``pyarrow.dictionary`` arrays that stored nulls as dictionary values ignored ``use_na_sentinel=True``, keeping the null in the returned ``uniques`` (:issue:`66490`)
