...
- Performance improvement in :func:`read_csv` with the C engine for files with many string columns or many columns overall, and for parallel reads generally: columns are now converted a cache-sized block of rows at a time instead of one full pass per column (:issue:`68380`)
