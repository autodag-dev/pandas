...
- :meth:`DataFrame.sum`, :meth:`DataFrame.prod`, :meth:`DataFrame.min`, :meth:`DataFrame.max` and :meth:`DataFrame.mean` with ``axis=1`` on a :class:`DataFrame` mixing bool columns with NumPy-backed numeric ones now return a numeric dtype rather than ``object``, matching what the same reduction returns when the bool columns are dropped; :meth:`DataFrame.min` and :meth:`DataFrame.max` on a mix of bool and complex columns previously raised ``TypeError`` and now return a complex result. The result is computed in the promoted NumPy dtype instead of with Python's arbitrary-precision arithmetic, so an integer ``sum`` or ``prod`` can now overflow and a floating-point result can lose precision (:issue:`51474`)
