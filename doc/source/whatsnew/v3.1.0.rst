...
- Bug in setting an out-of-range scalar into a numpy-nullable :class:`Series`, :class:`DataFrame` column or :class:`~pandas.api.extensions.ExtensionArray`; the value was either silently stored wrapped-around (``np.int64(-1)`` became ``255`` in ``UInt8``) or, depending on the entry point, raised ``OverflowError``, and both now raise ``TypeError``. :meth:`Series.fillna`, :meth:`Series.where` and :meth:`Series.mask` were affected the same way, as were :meth:`Index.fillna`, :meth:`Index.where` and :meth:`Index.putmask`, which upcast to a common dtype rather than raising (:issue:`48867`)
