...
- Bug in :meth:`DataFrame.iloc` setitem writing the wrong cells or raising ``IndexError`` or ``ValueError``, and :meth:`DataFrame.loc` setitem raising ``IndexError``, when the column indexer reordered the columns (e.g. ``df.iloc[[2, 0], ::-1] = value``) or the row indexer was a ``range``, tuple, nullable array or two-dimensional array, on a :class:`DataFrame` whose values are referenced by another object (:issue:`65446`)
