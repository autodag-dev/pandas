...
- Performance improvement in :class:`.DataFrameGroupBy` and :class:`.SeriesGroupBy` reductions for :class:`ArrowDtype` decimal and Arrow-backed string columns with PyArrow 23 or later, by placing the group results with PyArrow's ``scatter`` instead of converting them to NumPy and back (:issue:`66625`)
