...
- Fixed bug in comparisons involving object dtype array-like, :class:`Series`, or :class:`DataFrame` containing ``pd.NA``. These now return ``pd.NA`` instead of ``False`` (:issue:`63328`)
