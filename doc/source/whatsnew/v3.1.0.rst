...
- Bug in :class:`SparseDtype` comparing equal to another :class:`SparseDtype` with a different ``fill_value`` when either fill value was NA, so :meth:`Series.astype`, :meth:`DataFrame.astype`, and :meth:`SparseArray.astype` silently ignored a requested ``fill_value``, :meth:`DataFrame.select_dtypes` matched columns with the wrong one, and :func:`concat` skipped the ``PerformanceWarning`` that says which fill value it kept (:issue:`68567`)
