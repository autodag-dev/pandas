...
- Bug in :class:`StringDtype` with PyArrow storage validating a fill value differently from item assignment, so :meth:`api.extensions.ExtensionArray.fillna` silently stringified a ``bytes``, PyArrow-scalar or non-string array value and :meth:`Series.where` fell back to ``object`` for a PyArrow string scalar; both now validate as item assignment does, which in turn accepts a PyArrow string scalar (:issue:`68638`)
...
- Bug in :meth:`Series.replace`, :meth:`Index.replace`, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.fillna`, :meth:`Series.clip`, :meth:`Series.update` and item assignment (and their :class:`DataFrame` counterparts) on a PyArrow-backed integer dtype silently truncating a value with a fractional part, e.g. replacing with ``1.5`` stored ``1``; these now raise ``TypeError``, and :meth:`Index.fillna`, :meth:`Index.where` and :meth:`Index.putmask` now upcast, both matching the nullable integer dtypes (:issue:`68638`)
