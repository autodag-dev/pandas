...
- Bug in setitem (e.g. :meth:`Series.iloc`), :meth:`Series.fillna`, :meth:`Series.where`, :meth:`Series.mask`, :meth:`Series.replace`, :meth:`Series.reindex` and their :class:`DataFrame` counterparts on an :class:`ArrowDtype` ``timestamp`` column silently shifting a :class:`Timestamp` or ``datetime.datetime`` by the UTC offset when the value and the column disagreed about being timezone-aware; these now raise ``TypeError`` (:issue:`69029`)
