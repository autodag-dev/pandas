...
- Bug in :meth:`DataFrame.iloc` and :meth:`DataFrame.loc` setitem broadcasting a 1-D value down rows instead of across the selected columns on an all-``datetime64`` or all-``timedelta64`` :class:`DataFrame` (e.g. ``df.iloc[[2, 0], ::-1] = value``), raising ``OutOfBoundsDatetime``, or silently writing the values to the wrong cells when the value's length matched the number of selected rows; a 2-D value was transposed the same way when both keys were list-like or boolean masks (:issue:`68521`)
