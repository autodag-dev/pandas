...
- Bug in :meth:`DataFrame.iloc` and :meth:`Series.iloc` setitem raising ``TypeError`` ("boolean value of NA is ambiguous") when assigning a nullable integer or floating :class:`Series` that held ``NA`` and had a non-default integer index into a masked (e.g. ``Int64``, ``Float64``) column (:issue:`62473`)
