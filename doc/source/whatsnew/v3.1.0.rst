...
- Deprecated the ``double_precision`` keyword in :meth:`DataFrame.to_json` and :meth:`Series.to_json`, and the ``precise_float`` keyword in :func:`read_json`. In a future version, floating point values will be written with the shortest representation that round-trips exactly and read with full precision. Round the data before writing to control the number of digits (:issue:`62464`)
