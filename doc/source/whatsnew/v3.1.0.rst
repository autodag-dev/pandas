...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` where a requested dtype whose conversion must start from the original text was applied only after pyarrow's type inference, giving results that differed from the other engines: values with leading zeros lost the zeros under a string dtype, ``dtype=object`` produced the inferred values instead of the raw strings, and ``dtype="category"`` produced inferred-value categories instead of string categories. Such columns are now parsed as strings. For a scalar ``dtype`` (e.g. ``dtype=str``) this additionally requires pyarrow 25.0 or newer (:issue:`9435`, :issue:`57666`, :issue:`58260`)
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"``, ``header=None`` and ``usecols`` where the selected columns were relabeled positionally and ordered as given in ``usecols``, instead of keeping their file positions as labels and their file order like the other engines (:issue:`67008`)
