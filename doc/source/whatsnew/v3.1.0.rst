...
For those reasons, the ``inplace`` keyword is deprecated for those methods which
do not work inplace. In practice, this are the methods :meth:`~DataFrame.drop`,
:meth:`~DataFrame.rename`, :meth:`~DataFrame.rename_axis`,
:meth:`~DataFrame.reset_index`, :meth:`~DataFrame.set_index`,
:meth:`~DataFrame.dropna`, :meth:`~DataFrame.drop_duplicates`,
:meth:`~DataFrame.sort_values`, :meth:`~DataFrame.sort_index`,
:meth:`~DataFrame.eval`, :meth:`~DataFrame.query`.
...
- Bug in adding a quarter-, half-year- or year-based offset (e.g. :class:`QuarterEnd`, :class:`HalfYearEnd`, :class:`YearEnd`) with ``n`` near ``2**63`` to a :class:`Timestamp` raising :class:`OutOfBoundsDatetime` naming a negative year for a positive shift (:issue:`66549`)
- Bug in adding a quarter-, half-year- or year-based offset (e.g. :class:`QuarterEnd`, :class:`HalfYearEnd`, :class:`YearEnd`) with a very large ``n`` to a :class:`DatetimeIndex` or :class:`Series` silently shifting by a wrapped number of months, e.g. ``YearEnd(2**62)`` behaving like ``n=0``, instead of raising :class:`OutOfBoundsDatetime` as the equivalent :class:`Timestamp` operation does (:issue:`66549`)
...
- Bug in :class:`Timedelta` constructor and :func:`to_timedelta` raising a bare ``OverflowError`` instead of :class:`OutOfBoundsTimedelta` for infinite or overflowing float input, which also prevented ``errors="coerce"`` from returning ``NaT`` (:issue:`66247`)
...
- Bug in :class:`Timedelta` constructor with keyword arguments raising a bare ``OverflowError`` for infinite float input, e.g. ``Timedelta(days=np.inf)``, instead of :class:`OutOfBoundsTimedelta` (:issue:`66823`)
- Bug in :func:`to_timedelta` with ``errors="coerce"`` raising :class:`OutOfBoundsTimedelta` instead of returning ``NaT`` for out-of-bounds or infinite values in float-dtype input, e.g. ``to_timedelta(np.array([1.5, 1e30]), unit="s", errors="coerce")``; this also affected :func:`to_datetime` assembling from a :class:`DataFrame` with a float time-unit column (:issue:`66823`)
