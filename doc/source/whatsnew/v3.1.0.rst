...
- :meth:`DataFrame.to_json` and :meth:`Series.to_json` with ``orient="table"`` no longer raise ``AttributeError`` for a :class:`MultiIndex` with non-string level names (:issue:`19129`)
