...
- Clarified the ``ValueError`` raised by :meth:`Index.reindex`, :meth:`Series.reindex`, and :meth:`DataFrame.reindex` on a duplicate-labeled axis to explain that reindexing to the exact same index is still allowed as a no-op; the error only applies when the target actually differs from the existing index (:issue:`8849`)
