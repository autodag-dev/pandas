...
- Bug in :meth:`Series.str.extract` and :meth:`Index.str.extract` with :class:`ArrowDtype` and a ``pat`` containing an unnamed capture group raising ``ValueError`` when no group was named, raising ``ArrowInvalid`` when named and unnamed groups were mixed, and silently returning an all-``NA`` column for each unnamed group when they were mixed and ``flags`` was passed, instead of labelling the unnamed groups by position (:issue:`67024`)
