...
- Fixed bug in :func:`read_csv` with ``engine="pyarrow"`` where passing a dict ``dtype`` that did not name the ``index_col`` column raised ``IndexError`` when ``dtype_backend="pyarrow"``, instead of leaving the index column to be inferred and applying the remaining ``dtype`` keys (:issue:`68041`)
