...
- Bug in ufunc calls such as ``np.maximum(series, 0, where=series > 2)`` where passing a :class:`Series`, :class:`DataFrame`, or other pandas object as the ``where`` argument recursed infinitely, raising ``RecursionError`` or crashing the interpreter, instead of masking the result; a ``where`` argument sharing the operands' dimensionality is now aligned to them by label, with unmatched labels treated as ``False``, matching :meth:`Series.where`/:meth:`DataFrame.where` (:issue:`60611`)
- Bug in ufunc calls with both ``out`` and ``where`` arguments where passing a :class:`Series` or :class:`DataFrame` as ``out`` raised ``TypeError`` from ``numpy.putmask`` instead of writing the masked result into it (:issue:`60611`)
