...
- Bug in :meth:`DatetimeIndex.tz_localize`, :meth:`Series.dt.tz_localize`, and :meth:`Timestamp.tz_localize` returning a result off by the size of the DST transition when ``nonexistent`` shifted a wall time out of a gap, for ``"shift_forward"``, ``"shift_backward"`` and timedelta shifts alike (:issue:`66820`)
