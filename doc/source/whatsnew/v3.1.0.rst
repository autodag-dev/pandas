...
- Bug in :meth:`Series.str.get_dummies` and :meth:`Index.str.get_dummies` giving a column to entries that carry no tag: an empty string or a pair of adjacent separators produced a ``""`` column with PyArrow-backed string dtypes and :class:`ArrowDtype`, and a missing value produced a ``"NaN"`` column with :class:`CategoricalDtype` that also swallowed a genuine ``"NaN"`` tag; all-missing input with PyArrow-backed string dtypes raised ``ValueError`` instead of returning no columns (:issue:`67553`)
