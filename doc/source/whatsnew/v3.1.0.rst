...
- Fixed bug in :class:`Series` and :class:`DataFrame` comparisons where ``None == None`` would incorrectly evaluate to ``False`` (:issue:`20442`)
...
- Bug in :func:`pd.array` silently wrapping around instead of raising when converting a nullable integer array to a nullable integer dtype that cannot hold its values (:issue:`55232`)
...
- Fixed bug in :func:`read_csv` with ``engine="c"`` or ``engine="python"`` where a value outside the range of the requested integer ``dtype`` (e.g. ``-1`` with ``dtype="UInt8"``) was silently wrapped around, or the requested ``dtype`` silently ignored, instead of raising (:issue:`55232`)
