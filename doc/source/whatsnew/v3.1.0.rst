...
- :func:`read_csv` and :func:`read_table` accept ``to_pandas_kwargs`` when ``engine="pyarrow"``, which are forwarded to :meth:`pyarrow.Table.to_pandas` to customize the conversion to pandas, e.g. ``split_blocks`` or ``self_destruct`` to reduce peak memory usage; this matches the existing ``to_pandas_kwargs`` argument of :func:`read_parquet` (:issue:`34823`)
...
- :func:`read_parquet` now raises an informative ``ValueError`` when ``to_pandas_kwargs`` contains ``types_mapper``, which pandas derives from ``dtype_backend`` and passes to :meth:`pyarrow.Table.to_pandas` itself, instead of a cryptic ``TypeError`` reporting multiple values for the keyword (:issue:`XXXXX`)
