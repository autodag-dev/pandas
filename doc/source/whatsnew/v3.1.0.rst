...
- Bug in :meth:`DataFrame.loc` raising ``NotImplementedError`` when setting a single-column DataFrame with an :class:`ExtensionArray` dtype using a boolean column indexer (e.g. ``df.loc[:, [True]]`` or ``df.loc[:, df.columns.isin(["A"])]``) (:issue:`66527`)
