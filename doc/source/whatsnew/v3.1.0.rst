...
- Bug in :meth:`DataFrame.__setitem__` silently corrupting the :class:`DataFrame` when the key matched several columns and ``value`` was a one-dimensional timezone-aware ``datetime64`` or ``period`` :class:`Series`; it now broadcasts across the matched columns as the other dtypes already did (:issue:`68445`)
- Bug in :meth:`DataFrame.__setitem__` silently corrupting the :class:`DataFrame`, or dropping columns of ``value``, when the key matched several columns (duplicate or :class:`MultiIndex` columns) and ``value`` was a two-dimensional array without one column per matched column; it now raises ``ValueError`` (:issue:`68445`)
...
- Bug in :meth:`DataFrame.isetitem` with several positions silently corrupting the :class:`DataFrame`, or dropping columns of ``value``, when a NumPy-dtype ``value`` did not have one column per position; it now raises ``ValueError``, and a scalar ``value`` now fills every selected column (:issue:`68445`)
