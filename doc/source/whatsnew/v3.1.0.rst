...
- Bug in ``np.logical_and``, ``np.logical_or`` and ``np.logical_xor`` on, or against, datetime-like data -- ``datetime64`` (including timezone-aware), ``timedelta64``, :class:`PeriodDtype`, any temporal :class:`ArrowDtype`, the :class:`SparseDtype` spellings of the first two, and the corresponding scalars -- returning a result instead of raising ``TypeError`` as ``&``, ``|`` and ``^`` do; values NumPy converts to ``object``, such as :class:`PeriodDtype` and timezone-aware ``datetime64``, came back in the result themselves, and ``np.logical_not`` returned all-``False`` for them (:issue:`68524`)
