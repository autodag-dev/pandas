...
- Deprecated pinning the group key to the ``name`` attribute of the group passed to a user-defined function in :meth:`.SeriesGroupBy.apply`, :meth:`.SeriesGroupBy.transform`, :meth:`.DataFrameGroupBy.apply`, :meth:`.DataFrameGroupBy.transform`, and :meth:`.DataFrameGroupBy.filter`. In a future version of pandas, the ``name`` attribute will no longer be set to the group key. When you need the key inside the function, iterate over the groupby object and combine the results, e.g. ``pd.concat({key: func(group, key) for key, group in gb})`` (:issue:`41090`)
