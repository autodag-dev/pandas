...
- Bug in :meth:`DatetimeIndex.tz_localize`, :meth:`Series.dt.tz_localize`, and :meth:`Timestamp.tz_localize` with a timedelta ``nonexistent`` shift returning a timestamp off by the DST offset whenever the shift was large enough to move the value across the transition; only one-hour shifts were correct (:issue:`66820`)
