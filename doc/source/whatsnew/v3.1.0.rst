...
- Bug in :func:`factorize` and groupby operations with a PyArrow ``dictionary`` dtype producing a group for a value absent from the data, e.g. after filtering, or splitting one value across two groups; ``uniques`` are now ordered by first appearance, as they are for every other dtype, rather than by the stored dictionary (:issue:`69024`)
