...
- Bug in comparisons between PyArrow-backed data and objects PyArrow cannot convert, such as :class:`Categorical`, :class:`Interval`, or ``DateOffset`` objects, raising an opaque ``pyarrow.ArrowInvalid`` or ``pyarrow.ArrowTypeError`` instead of comparing elementwise for ``==``/``!=`` and raising an informative ``TypeError`` for ordered comparisons, as the NumPy-nullable dtypes do (:issue:`62682`)
