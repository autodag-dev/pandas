...
- Fixed bug where :class:`DataFrame` arithmetic, :meth:`DataFrame.reindex`, and :meth:`DataFrame.align` cast newly-introduced all-NA columns to ``float64``/``NaN`` instead of preserving the existing dtype with its NA value (e.g. ``NaT`` for datetime/timedelta, ``pd.NA`` for pyarrow/nullable dtypes); the dtype is preserved when all existing columns share it (:issue:`59529`, :issue:`63288`)
