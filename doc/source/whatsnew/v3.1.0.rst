...
- Fixed regression in :func:`read_csv` where ``sep=None`` (automatic separator detection) reached the ``c`` engine and raised ``TypeError: object of type 'NoneType' has no len()``; the default engine now falls back to the ``python`` engine with a ``ParserWarning`` and an explicit non-``python`` engine raises an informative ``ValueError`` (:issue:`66639`)
