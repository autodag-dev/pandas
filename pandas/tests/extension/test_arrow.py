...
def test_str_extract_non_symbolic(expand):
    # GH#67024 an unnamed group is labelled by position, as for other dtypes
    ser = pd.Series(["a1", "b2", "c3"], dtype=ArrowDtype(pa.string()))
    result = ser.str.extract(r"[ab](\d)", expand=expand)
    expected: pd.DataFrame | pd.Series
    if expand:
        expected = pd.DataFrame({0: ArrowExtensionArray(pa.array(["1", "2", None]))})
    else:
        expected = pd.Series(ArrowExtensionArray(pa.array(["1", "2", None])))
    tm.assert_equal(result, expected)


def test_str_extract_non_symbolic_multiple():
    # GH#67024
    ser = pd.Series(["a1", "b2", "c3"], dtype=ArrowDtype(pa.string()))
    result = ser.str.extract(r"([ab])(\d)")
    expected = pd.DataFrame(
        {
            0: ArrowExtensionArray(pa.array(["a", "b", None])),
            1: ArrowExtensionArray(pa.array(["1", "2", None])),
        }
    )
    tm.assert_frame_equal(result, expected)


def test_str_extract_mixed_symbolic():
    # GH#67024 a pattern may mix named and unnamed groups
    ser = pd.Series(["a1", "b2", "c3"], dtype=ArrowDtype(pa.string()))
    result = ser.str.extract(r"(?P<letter>[ab])(\d)")
    expected = pd.DataFrame(
        {
...
def test_str_extract_non_symbolic_flags():
    # GH#67024 the `re` fallback taken for flags also handles unnamed groups
    ser = pd.Series(["A1", "b2", "c3"], dtype=ArrowDtype(pa.string()))
    result = ser.str.extract(r"([ab])(\d)", flags=re.IGNORECASE)
    expected = pd.DataFrame(
        {
            0: ArrowExtensionArray(pa.array(["A", "b", None])),
            1: ArrowExtensionArray(pa.array(["1", "2", None])),
