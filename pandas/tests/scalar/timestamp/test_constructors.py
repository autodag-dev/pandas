...
class TestTimestampConstructorFoldKeyword:
    ...
    def test_timestamp_constructor_pytz_fold_raise(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(datetime(2019, 10, 27, 0, 30, 0, 0), tz=tz, fold=0)
    ...
    def test_timestamp_constructor_fold_conflict(self, ts_input, fold):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(ts_input=ts_input, fold=fold)
    ...
    def test_timestamp_constructor_string_tz_respects_fold(self):
        ...
        ts0 = pd.Timestamp(naive, tz="America/New_York", fold=0)
        ts1 = pd.Timestamp(naive, tz="America/New_York", fold=1)
        assert ts0 != ts1
        assert (ts1 - ts0) == pd.Timedelta(hours=1)
...
class TestTimestampConstructorPositionalAndKeywordSupport:
    def test_constructor_positional(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2000, 0, 1)
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2000, 13, 1)
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2000, 1, 0)
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2000, 1, 32)

        # see gh-11630
        assert repr(pd.Timestamp(2015, 11, 12)) == repr(pd.Timestamp("20151112"))
        assert repr(pd.Timestamp(2015, 11, 12, 1, 2, 3, 999999)) == repr(
            pd.Timestamp("2015-11-12 01:02:03.999999")
        )
    ...
    def test_constructor_keyword(self):
        ...
        msg = "month must be in 1..12"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(year=2000, month=0, day=1)
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(year=2000, month=13, day=1)
        ...
        if PY314:
            ...
            msg = "day is out of range for month"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(year=2000, month=1, day=0)
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(year=2000, month=1, day=32)

        assert repr(pd.Timestamp(year=2015, month=11, day=12)) == repr(
            pd.Timestamp("20151112")
        )
        ...
        ) == repr(pd.Timestamp("2015-11-12 01:02:03.999999"))
    ...
    def test_invalid_date_kwarg_with_string_input(self, arg):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2010-10-10 12:59:59.999999999", **kwarg)
    ...
    def test_invalid_date_kwarg_names_the_offending_arguments(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(datetime(2020, 12, 31), hour=5, minute=3)
    ...
    def test_components_without_year_report_value_form(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2020, month=12, day=31)
    ...
    def test_int_value_with_year_kwarg_reads_as_components(self):
        ...
        assert pd.Timestamp(2020, year=1, month=1) == pd.Timestamp(2020, 1, 1)

    @pytest.mark.parametrize(
        "value", [datetime(2020, 12, 31), pd.Timestamp("2020-12-31")]
    )
    def test_nanosecond_kwarg_allowed_with_pydatetime_input(self, value):
        ...
        result = pd.Timestamp(value, nanosecond=5)
    ...
    def test_nanosecond_kwarg_still_dropped_for_other_value_forms(self, value):
        ...
        assert pd.Timestamp(value, nanosecond=5).nanosecond == 0
    ...
    def test_constructor_positional_with_tzinfo(self):
        # GH#31929
        ts = pd.Timestamp(2020, 12, 31, tzinfo=UTC)
        expected = pd.Timestamp("2020-12-31", tzinfo=UTC)
        assert ts == expected
    ...
    def test_constructor_positional_keyword_mixed_with_tzinfo(self):
        ...
        ts = pd.Timestamp(2020, 12, 31, tzinfo=UTC, nanosecond=4)
        expected = pd.Timestamp("2020-12-31", tz=UTC) + pd.Timedelta(nanoseconds=4)
        assert ts == expected
    ...
    def test_constructor_positional_keyword_no_gap(self):
        ...
        assert pd.Timestamp(2020, 12, 31, day=5) == pd.Timestamp(2020, 12, 31, 5)
    ...
    def test_constructor_positional_tzinfo_slot(self):
        ...
        result = pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, tz)
        assert result == pd.Timestamp(datetime(2000, 1, 2, 3, 4, 5, 6, tz))
        assert result.tzinfo is tz

        # the 8th slot is `microsecond`, so the keyword spelling lands there too
        assert pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, microsecond=tz) == result
    ...
    def test_constructor_positional_tzinfo_slot_invalid(self):
        ...
        with pytest.raises(TypeError, match=msg):
            pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, 7)
    ...
    def test_constructor_positional_tzinfo_slot_with_tz_keyword(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, UTC, tz="UTC")
    ...
    def test_datetime_reconstruction_keeps_tzinfo(self):
        ...
        tz = zoneinfo.ZoneInfo("US/Eastern")
        ts = pd.Timestamp("2020-07-01 12:00:00.000000123", tz=tz)
        assert ts.unit == "ns"
    ...
    def test_copy_replace_keeps_tzinfo(self):
        ...
        assert replaced == pd.Timestamp("2020-07-02 12:00", tz=tz)
...
class TestTimestampClassMethodConstructors:
    ...
    def test_utcfromtimestamp_deprecated(self):
        ...
        with tm.assert_produces_warning(Pandas4Warning, match=msg):
            pd.Timestamp.utcfromtimestamp(43)
    ...
    def test_constructor_fromisocalendar(self):
        # GH#30395
        expected_timestamp = pd.Timestamp("2000-01-03 00:00:00")
        expected_stdlib = datetime.fromisocalendar(2000, 1, 1)
        result = pd.Timestamp.fromisocalendar(2000, 1, 1)
    ...
    def test_constructor_fromisoformat_preserves_tz(self, date_string):
        ...
        result = pd.Timestamp.fromisoformat(date_string)
        expected_stdlib = datetime.fromisoformat(date_string)
        assert isinstance(result, pd.Timestamp)
        assert result.tzinfo is not None
        assert result == expected_stdlib
        assert result == pd.Timestamp(date_string)

    def test_constructor_fromisoformat_naive(self):
        # GH#56398 the tz-naive case should remain tz-naive
        result = pd.Timestamp.fromisoformat("2023-01-15T10:30:00")
        assert isinstance(result, pd.Timestamp)
        assert result.tzinfo is None
        assert result == pd.Timestamp("2023-01-15T10:30:00")

    def test_constructor_fromisoformat_roundtrip_isoformat(self):
        # GH#56398 fromisoformat(ts.isoformat()) should round-trip
        original = pd.Timestamp.now("UTC")  # noqa: TID251
        result = pd.Timestamp.fromisoformat(original.isoformat())
    ...
    def test_constructor_fromordinal(self):
        ...
        ts = pd.Timestamp.fromordinal(base.toordinal())
        assert base == ts
        assert base.toordinal() == ts.toordinal()

        ts = pd.Timestamp.fromordinal(base.toordinal(), tz="US/Eastern")
        assert pd.Timestamp("2000-01-01", tz="US/Eastern") == ts
        assert base.toordinal() == ts.toordinal()

        # GH#3042
        dt = datetime(2011, 4, 16, 0, 0)
        ts = pd.Timestamp.fromordinal(dt.toordinal())
        assert ts.to_pydatetime() == dt

        # with a tzinfo
        stamp = pd.Timestamp("2011-4-16", tz="US/Eastern")
        dt_tz = stamp.to_pydatetime()
        ts = pd.Timestamp.fromordinal(dt_tz.toordinal(), tz="US/Eastern")
        assert ts.to_pydatetime() == dt_tz

    def test_now(self):
        # GH#9000
        ts_from_string = pd.Timestamp("now")  # current-time-ok
        ts_from_method = pd.Timestamp.now()  # noqa: TID251
        ts_datetime = datetime.now()  # noqa: TID251

        ts_from_string_tz = pd.Timestamp("now", tz="US/Eastern")  # current-time-ok
        ts_from_method_tz = pd.Timestamp.now(tz="US/Eastern")  # noqa: TID251
    ...
    def test_today(self):
        ts_from_string = pd.Timestamp("today")  # current-time-ok
        ts_from_method = pd.Timestamp.today()  # noqa: TID251
        ts_datetime = datetime.today()  # noqa: TID251

        ts_from_string_tz = pd.Timestamp("today", tz="US/Eastern")  # current-time-ok
        ts_from_method_tz = pd.Timestamp.today(tz="US/Eastern")  # noqa: TID251
...
class TestTimestampResolutionInference:
    ...
    def test_constructor_str_infer_reso(self):
        ...
        with tm.assert_produces_warning(
            ...
            ts = pd.Timestamp("2015Q1")
        assert ts.unit == "us"

        # dateutil_parse path
        ts = pd.Timestamp("2016-01-01 1:30:01 PM")
        assert ts.unit == "us"

        ts = pd.Timestamp("2016 June 3 15:25:01.345")
        assert ts.unit == "us"

        ts = pd.Timestamp("300-01-01")
        assert ts.unit == "us"

        ts = pd.Timestamp("300 June 1:30:01.300")
        assert ts.unit == "us"

        # dateutil path -> don't drop trailing zeros
        ts = pd.Timestamp("01-01-2013T00:00:00.000000000+0000")
        assert ts.unit == "ns"

        ts = pd.Timestamp("2016/01/02 03:04:05.001000 UTC")
        assert ts.unit == "us"

        # higher-than-nanosecond -> we drop the trailing bits
        ts = pd.Timestamp("01-01-2013T00:00:00.000000002100+0000")
        assert ts == pd.Timestamp("01-01-2013T00:00:00.000000002+0000")
        assert ts.unit == "ns"

        # GH#56208 minute reso through the ISO8601 path with tz offset
        ts = pd.Timestamp("2020-01-01 00:00+00:00")
        assert ts.unit == "us"
    ...
    def test_now_today_unit(self, method):
        # GH#55879
        ts_from_method = getattr(pd.Timestamp, method)()
        ts_from_string = pd.Timestamp(method)
...
class TestTimestampConstructors:
    ...
    def test_construct_from_string_invalid_raises(self):
        ...
        with pytest.raises(ValueError, match="gives an invalid tzoffset"):
            pd.Timestamp("200622-12-31")

    def test_constructor_from_iso8601_str_with_offset_reso(self):
        # GH#49737
        ts = pd.Timestamp("2016-01-01 04:05:06-01:00")
        assert ts.unit == "us"

        ts = pd.Timestamp("2016-01-01 04:05:06.000-01:00")
        assert ts.unit == "us"

        ts = pd.Timestamp("2016-01-01 04:05:06.000000-01:00")
        assert ts.unit == "us"

        ts = pd.Timestamp("2016-01-01 04:05:06.000000001-01:00")
        assert ts.unit == "ns"
    ...
    def test_constructor_datetime64_with_tz(self):
        ...
        ts = pd.Timestamp(dt, tz=tzstr)

        alt = pd.Timestamp(dt).tz_localize(tzstr)
        assert ts == alt
        assert ts.hour == 5
    ...
    def test_constructor(self):
        ...
        tests = [
            ...
            (
                ...
                pd.Timestamp("2014-07-01 09:00:00.000000005"),
                base_expected + 5,
            ),
        ]
        ...
        for date_str, date_obj, expected in tests:
            for result in [pd.Timestamp(date_str), pd.Timestamp(date_obj)]:
            ...
            for tz, offset in timezones:
                for result in [
                    ...
                    if tz is not None:
                        result = pd.Timestamp(result).tz_convert("UTC")
                    else:
    ...
    def test_constructor_with_stringoffset(self):
        ...
        for date_str, expected in tests:
            for result in [pd.Timestamp(date_str)]:
                # only with timestring
                assert result.as_unit("ns")._value == expected
            ...
            # with timezone
            for tz, offset in timezones:
                result = pd.Timestamp(date_str, tz=tz)
        ...
        result = pd.Timestamp("2013-11-01 00:00:00-0500", tz="America/Chicago")
        assert result._value == pd.Timestamp("2013-11-01 05:00")._value
        expected = "Timestamp('2013-11-01 00:00:00-0500', tz='America/Chicago')"
        assert repr(result) == expected
        assert result == eval(repr(result), dict(vars(pd)))

        # This should be 2013-11-01 05:00 in UTC
        # converted to Tokyo tz (+09:00)
        result = pd.Timestamp("2013-11-01 00:00:00-0500", tz="Asia/Tokyo")
        assert result._value == pd.Timestamp("2013-11-01 05:00")._value
        ...
        result = pd.Timestamp("2015-11-18 15:45:00+05:45", tz="Asia/Katmandu")
        assert result._value == pd.Timestamp("2015-11-18 10:00")._value
        ...
        result = pd.Timestamp("2015-11-18 15:30:00+05:30", tz="Asia/Kolkata")
        assert result._value == pd.Timestamp("2015-11-18 10:00")._value
    ...
    def test_constructor_invalid(self):
        msg = "Cannot convert input"
        with pytest.raises(TypeError, match=msg):
            pd.Timestamp(slice(2))
        msg = "Cannot convert Period"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(pd.Period("1000-01-01"))
    ...
    def test_constructor_invalid_tz(self):
        ...
        with pytest.raises(TypeError, match=msg):
            pd.Timestamp("2017-10-22", tzinfo="US/Eastern")

        msg = "at most one of"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2017-10-22", tzinfo=UTC, tz="UTC")
    ...
    def test_constructor_tz_or_tzinfo(self):
        ...
        stamps = [
            pd.Timestamp(year=2017, month=10, day=22, tz="UTC"),
            pd.Timestamp(year=2017, month=10, day=22, tzinfo=UTC),
            pd.Timestamp(year=2017, month=10, day=22, tz=UTC),
            pd.Timestamp(datetime(2017, 10, 22), tzinfo=UTC),
            pd.Timestamp(datetime(2017, 10, 22), tz="UTC"),
            pd.Timestamp(datetime(2017, 10, 22), tz=UTC),
        ]
    ...
    @pytest.mark.parametrize(
        "result",
        [
            pd.Timestamp(datetime(2000, 1, 2, 3, 4, 5, 6), nanosecond=1),
            pd.Timestamp(
                year=2000,
                month=1,
                day=2,
            ...
            pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, None, nanosecond=1),
            pd.Timestamp(2000, 1, 2, 3, 4, 5, 6, tz=UTC, nanosecond=1),
        ],
    )
    def test_constructor_nanosecond(self, result):
        # GH 18898
        # As of 2.0 (GH 49416), nanosecond should not be accepted positionally
        expected = pd.Timestamp(datetime(2000, 1, 2, 3, 4, 5, 6), tz=result.tz)
        expected = expected + pd.Timedelta(nanoseconds=1)
        assert result == expected
    ...
    def test_constructor_invalid_Z0_isostring(self, z):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(f"2014-11-02 01:00{z}")

    def test_out_of_bounds_integer_value(self):
        # GH#26651 check that we raise OutOfBoundsDatetime, not OverflowError
        msg = str(pd.Timestamp.max._value * 2)
        with pytest.raises(OutOfBoundsDatetime, match=msg):
            pd.Timestamp(pd.Timestamp.max._value * 2)
        msg = str(pd.Timestamp.min._value * 2)
        with pytest.raises(OutOfBoundsDatetime, match=msg):
            pd.Timestamp(pd.Timestamp.min._value * 2)
    ...
    def test_out_of_bounds_value(self):
        ...
        min_ts_us = np.datetime64(pd.Timestamp.min).astype("M8[us]") + one_us
        max_ts_us = np.datetime64(pd.Timestamp.max).astype("M8[us]")

        # No error for the min/max datetimes
        pd.Timestamp(min_ts_us)
        pd.Timestamp(max_ts_us)

        # We used to raise on these before supporting non-nano
        us_val = NpyDatetimeUnit.NPY_FR_us.value
        assert pd.Timestamp(min_ts_us - one_us)._creso == us_val
        assert pd.Timestamp(max_ts_us + one_us)._creso == us_val
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(too_low)

        # One us more than the maximum is an error
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(too_high)

    def test_out_of_bounds_string(self):
        msg = "Cannot cast .* to unit='ns' without overflow"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("1676-01-01").as_unit("ns")
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2263-01-01").as_unit("ns")

        ts = pd.Timestamp("2263-01-01")
        assert ts.unit == "us"

        ts = pd.Timestamp("1676-01-01")
        assert ts.unit == "us"
    ...
    def test_barely_out_of_bounds(self):
        ...
        with pytest.raises(OutOfBoundsDatetime, match=msg):
            pd.Timestamp("2262-04-11 23:47:16.854775808")
    ...
    def test_construct_timestamp_near_dst(self, offset):
        # GH 20854
        expected = pd.Timestamp(f"2016-10-30 03:00:00{offset}", tz="Europe/Helsinki")
        result = pd.Timestamp(expected).tz_convert("Europe/Helsinki")
        assert result == expected
    ...
    def test_construct_with_different_string_format(self, arg):
        # GH 12064
        result = pd.Timestamp(arg)
        expected = pd.Timestamp(datetime(2013, 1, 1), tz=timezone(timedelta(hours=9)))
        assert result == expected

    def test_comma_separated_fractional_seconds(self):
        # GH#59256 - comma as decimal separator should parse correctly
        ts_comma = pd.Timestamp("2024-06-17T18:57:43,567")
        ts_dot = pd.Timestamp("2024-06-17T18:57:43.567")
    ...
    def test_raise_tz_and_tzinfo_in_datetime_input(self, box):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(box(**kwargs), tz="US/Pacific")
        msg = "Cannot pass a datetime or Timestamp"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(box(**kwargs), tzinfo=zoneinfo.ZoneInfo("US/Pacific"))

    def test_dont_convert_dateutil_utc_to_default_utc(self):
        result = pd.Timestamp(datetime(2018, 1, 1), tz=tzutc())
        expected = pd.Timestamp(datetime(2018, 1, 1)).tz_localize(tzutc())
        assert result == expected
    ...
    def test_constructor_subclassed_datetime(self):
        ...
        result = pd.Timestamp(data)
        expected = pd.Timestamp(2000, 1, 1)
        assert result == expected

    def test_timestamp_constructor_tz_utc(self):
        utc_stamp = pd.Timestamp("3/11/2012 05:00", tz="utc")
        assert utc_stamp.tzinfo is UTC
        assert utc_stamp.hour == 5

        utc_stamp = pd.Timestamp("3/11/2012 05:00").tz_localize("utc")
        assert utc_stamp.hour == 5

    def test_timestamp_to_datetime_tzoffset(self):
        tzinfo = tzoffset(None, 7200)
        expected = pd.Timestamp("3/11/2012 04:00", tz=tzinfo)
        result = pd.Timestamp(expected.to_pydatetime())
    ...
    def test_timestamp_constructor_near_dst_boundary(self):
        ...
        for tz in ["Europe/Brussels", "Europe/Prague"]:
            result = pd.Timestamp("2015-10-25 01:00", tz=tz)
            expected = pd.Timestamp("2015-10-25 01:00").tz_localize(tz)
            assert result == expected

            msg = "Cannot infer dst time from 2015-10-25 02:00:00"
            with pytest.raises(ValueError, match=msg):
                pd.Timestamp("2015-10-25 02:00", tz=tz)

        result = pd.Timestamp("2017-03-26 01:00", tz="Europe/Paris")
        expected = pd.Timestamp("2017-03-26 01:00").tz_localize("Europe/Paris")
        assert result == expected

        msg = "2017-03-26 02:00"
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2017-03-26 02:00", tz="Europe/Paris")

        # GH#11708
        naive = pd.Timestamp("2015-11-18 10:00:00")
        result = naive.tz_localize("UTC").tz_convert("Asia/Kolkata")
        expected = pd.Timestamp("2015-11-18 15:30:00+0530", tz="Asia/Kolkata")
        assert result == expected

        # GH#15823
        result = pd.Timestamp("2017-03-26 00:00", tz="Europe/Paris")
        expected = pd.Timestamp("2017-03-26 00:00:00+0100", tz="Europe/Paris")
        assert result == expected

        result = pd.Timestamp("2017-03-26 01:00", tz="Europe/Paris")
        expected = pd.Timestamp("2017-03-26 01:00:00+0100", tz="Europe/Paris")
        assert result == expected
        ...
        result = pd.Timestamp("2017-03-26 02:00:00+0100", tz="Europe/Paris")
        naive = pd.Timestamp(result.as_unit("ns")._value)
        expected = naive.tz_localize("UTC").tz_convert("Europe/Paris")
        assert result == expected

        result = pd.Timestamp("2017-03-26 03:00", tz="Europe/Paris")
        expected = pd.Timestamp("2017-03-26 03:00:00+0200", tz="Europe/Paris")
        assert result == expected
    ...
    def test_timestamp_constructed_by_date_and_tz(self, tz):
        ...
        result = pd.Timestamp(date(2012, 3, 11), tz=tz)

        expected = pd.Timestamp("3/11/2012", tz=tz)
    ...
    def test_explicit_tz_none(self):
        ...
        with pytest.raises(ValueError, match=msg):
            pd.Timestamp(datetime(2022, 1, 1, tzinfo=UTC), tz=None)

        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2022-01-01 00:00:00", tzinfo=UTC, tz=None)

        with pytest.raises(ValueError, match=msg):
            pd.Timestamp("2022-01-01 00:00:00-0400", tz=None)
...
def test_constructor_ambiguous_dst():
    ...
    ts = pd.Timestamp(1382835600000000000, tz="dateutil/Europe/London")
    expected = ts._value
    result = pd.Timestamp(ts)._value
    assert result == expected
...
def test_constructor_before_dst_switch(epoch):
    ...
    ts = pd.Timestamp(epoch, tz="dateutil/America/Los_Angeles")
    result = ts.tz.dst(ts)
    expected = timedelta(seconds=0)
    assert pd.Timestamp(ts)._value == epoch
    assert result == expected
...
def test_timestamp_nano_range(nano):
    ...
    with pytest.raises(ValueError, match="nanosecond must be in 0..999"):
        pd.Timestamp(year=2022, month=1, day=1, nanosecond=nano)
...
def test_timestamp_constructor_np_str():
    ...
    result = pd.Timestamp(np.str_("2023-01-01"))
    expected = pd.Timestamp("2023-01-01")
    assert result == expected

    result = pd.Timestamp(np.str_("2023-01-01 12:34:56"))
    expected = pd.Timestamp("2023-01-01 12:34:56")
    assert result == expected


@pytest.mark.parametrize(
    "na_value", [None, np.nan, np.datetime64("NaT", "ns"), pd.NaT, pd.NA]
)
def test_timestamp_constructor_na_value(na_value):
    # GH45481
    result = pd.Timestamp(na_value)
    expected = pd.NaT
    assert result is expected
...
def test_timestamp_constructor_na_value_with_components(na_value, kwargs):
    ...
    assert pd.Timestamp(na_value, **kwargs) is pd.NaT
...
def test_timestamp_constructor_na_value_still_validates(na_value):
    ...
    with pytest.raises(ValueError, match="nanosecond must be in 0..999"):
        pd.Timestamp(na_value, nanosecond=5000)

    with pytest.raises(zoneinfo.ZoneInfoNotFoundError, match="Not/AZone"):
        pd.Timestamp(na_value, tz="Not/AZone")
...
def test_timestamp_iso8601_offset_out_of_bounds(tz):
    ...
    with pytest.raises(OutOfBoundsDatetime, match=msg):
        pd.Timestamp("2262-04-11T20:00:00.000000000-11:00", tz=tz)
...
def test_constructor_naive_datetime_renders_nat_sentinel():
    ...
    with pytest.raises(OutOfBoundsDatetime, match=msg):
        pd.Timestamp(datetime(1677, 9, 21, 0, 12, 43, 145224), nanosecond=192)
...
def test_constructor_datetime64_tz_shift_hits_nat_sentinel():
    ...
    with pytest.raises(OutOfBoundsDatetime, match=msg):
        pd.Timestamp(np.datetime64(SENTINEL_PLUS_1H.replace(" ", "T")), tz=PLUS_1H)
...
def test_constructor_aware_datetime_offset_overflow():
    ...
    with pytest.raises(OutOfBoundsDatetime, match=msg):
        pd.Timestamp(
            datetime(2262, 4, 11, 23, 47, 16, 854775, tzinfo=tz), nanosecond=807
        )
...
def test_constructor_nat_sentinel_neighbours():
    ...
    assert pd.Timestamp("1677-09-21 00:12:43.145224193")._value == -(2**63) + 1
    assert pd.Timestamp.min._value == -(2**63) + 1
    assert pd.Timestamp(pd.NaT) is pd.NaT
...
def test_constructor_quarterly_string_deprecated(date_str):
    ...
    with tm.assert_produces_warning(
        ...
        result = pd.Timestamp(date_str)

    with tm.assert_produces_warning(None):
        expected = pd.Period(date_str).to_timestamp()
    assert result == expected
