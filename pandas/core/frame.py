        ...
        if axis is None:
            ...
            if df.shape[1]:
                ...
                if isinstance(dtype, ExtensionDtype):
                    ...
                    bias = op_kwargs.pop("bias", False)
                    try:
                        if bias and name in ("skew", "kurt"):
                            # group_skew/group_kurt do not yet support the
                            # bias parameter; fall back below.
                            raise NotImplementedError(
                                f"bias=True is not yet supported for axis=1 "
                                f"{name} on extension-array dtypes"
                            )
